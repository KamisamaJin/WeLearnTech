window.lessonGuideL3LessonChunks ||= {};
window.lessonGuideL3LessonChunks["l3-10"] = {
    "id": "l3-10",
    "number": 10,
    "titleKo": "취업",
    "titleZh": "就业",
    "pages": "130-141",
    "status": "draft",
    "sourceFile": "kiip-3-textbook.pdf",
    "progress": {
        "vocabulary": "audited-p130-p141",
        "dialogue": "reviewed-draft",
        "culture": "reviewed-draft"
    },
    "theme": "学习求职、招聘广告、面试和简历填写，能说明就业准备并理解韩国工资和税金制度。",
    "goals": [
        "能说出自己想找的工作、公司条件和求职流程。",
        "能用 동-기 위해서 表达目的。",
        "能用 동-어 놓다 表达提前完成并保持结果。",
        "能在求职对话和面试中说明准备内容。",
        "能阅读招聘广告、填写简历，并理解工资扣除项目。"
    ],
    "vocabularySources": [
        {
            "page": 130,
            "label": "단원 목표",
            "note": "单元主题、词汇目标、语法、活动和文化标题。"
        },
        {
            "page": 132,
            "label": "어휘",
            "note": "工作种类、想去的公司条件和求职流程。"
        },
        {
            "page": 133,
            "label": "문법: 동-기 위해서",
            "note": "说明做某事的目的和准备。"
        },
        {
            "page": 134,
            "label": "문법: 동-어 놓다",
            "note": "说明事先完成并保留结果。"
        },
        {
            "page": 135,
            "label": "말하기",
            "note": "咨询韩国就业准备和资格证建议。"
        },
        {
            "page": 136,
            "label": "듣기/발음",
            "note": "面试问题、경력 증명서发音和听力词。"
        },
        {
            "page": 137,
            "label": "읽기 전",
            "note": "招聘广告字段、提交材料和证明书词汇。"
        },
        {
            "page": 138,
            "label": "읽기",
            "note": "英语教师和文具店员工招聘广告。"
        },
        {
            "page": 139,
            "label": "쓰기",
            "note": "简历项目、学历、经历和资格证字段。"
        },
        {
            "page": 140,
            "label": "문화와 정보",
            "note": "급여와 세금文化文章里的工资、扣除和税金词。"
        },
        {
            "page": 141,
            "label": "배운 어휘 확인",
            "note": "补充复习词表中的就业、公司条件和求职材料词。"
        }
    ],
    "grammar": [
        {
            "pattern": "동-기 위해서",
            "zh": "为了……",
            "page": 133,
            "guide": "前面接动词词干，表示后面动作的目的或意图。中文常译为“为了……”。和 취업, 자격증, 준비하다 等求职话题特别常一起出现。",
            "examples": [
                {
                    "ko": "한국 회사에 취직하기 위해서 컴퓨터를 배우고 있어요.",
                    "zh": "为了进韩国公司工作，我正在学电脑。"
                },
                {
                    "ko": "토픽 시험에 합격하기 위해 열심히 공부하고 있어요.",
                    "zh": "为了通过TOPIK考试，我正在努力学习。"
                },
                {
                    "ko": "일하는 엄마들은 아이 맡길 곳을 찾기 위해 노력하고 있다.",
                    "zh": "上班的妈妈们为了寻找托管孩子的地方而努力。"
                }
            ]
        },
        {
            "pattern": "동-어 놓다",
            "zh": "事先……好；……着",
            "page": 134,
            "guide": "表示某个动作已经完成，并且结果保持着。求职场景里可说 이력서를 써 놓다, 자격증을 따 놓다, 서류를 준비해 놓다。",
            "examples": [
                {
                    "ko": "미리 이력서를 써 놓는 것이 좋을 거예요.",
                    "zh": "最好提前把简历写好。"
                },
                {
                    "ko": "휴가철이라서 비행기표를 미리 예약해 놓았어요.",
                    "zh": "因为是假期，我已经提前订好了机票。"
                },
                {
                    "ko": "숙제를 미리 해 놓아서 마음이 편해요.",
                    "zh": "因为提前把作业做好了，心里很轻松。"
                }
            ]
        }
    ],
    "vocabulary": [
        {
            "ko": "취업",
            "zh": "就业",
            "pos": "명사",
            "page": 130,
            "source": "단원 제목",
            "guide": "일자리를 얻어 직장에 들어가는 것.",
            "exampleKo": "취업을 준비하고 있어요.",
            "exampleZh": "我正在准备就业。"
        },
        {
            "ko": "취직하다",
            "zh": "就职；找到工作",
            "pos": "동사",
            "page": 130,
            "source": "어휘 목표",
            "guide": "회사나 기관에 들어가 일을 시작하다.",
            "exampleKo": "한국 회사에 취직하고 싶어요.",
            "exampleZh": "我想进入韩国公司工作。"
        },
        {
            "ko": "구직 활동",
            "zh": "求职活动",
            "pos": "명사구",
            "page": 130,
            "source": "활동 목표",
            "guide": "일자리를 찾고 지원하는 활동.",
            "exampleKo": "구직 활동에 대해 조언했어요.",
            "exampleZh": "就求职活动给了建议。"
        },
        {
            "ko": "이력서 작성",
            "zh": "写简历",
            "pos": "명사구",
            "page": 130,
            "source": "활동 목표",
            "guide": "개인 정보, 학력, 경력 등을 문서로 쓰는 일.",
            "exampleKo": "이력서 작성을 연습했어요.",
            "exampleZh": "练习了写简历。"
        },
        {
            "ko": "급여와 세금",
            "zh": "工资和税金",
            "pos": "명사구",
            "page": 130,
            "source": "문화 제목",
            "guide": "월급과 월급에서 내는 세금을 함께 말한다.",
            "exampleKo": "급여와 세금에 대해 읽었어요.",
            "exampleZh": "阅读了工资和税金。"
        },
        {
            "ko": "일을 구하다",
            "zh": "找工作",
            "pos": "표현",
            "page": 132,
            "source": "어휘",
            "guide": "일자리를 찾는다는 뜻이다.",
            "exampleKo": "한국에서 어떤 일을 구해요?",
            "exampleZh": "你在韩国找什么工作？"
        },
        {
            "ko": "학원 강사",
            "zh": "培训班讲师",
            "pos": "명사",
            "page": 132,
            "source": "어휘",
            "guide": "학원에서 학생을 가르치는 사람.",
            "exampleKo": "학원 강사로 일하고 싶어요.",
            "exampleZh": "我想做培训班讲师。"
        },
        {
            "ko": "다문화 언어 강사",
            "zh": "多元文化语言讲师",
            "pos": "명사구",
            "page": 132,
            "source": "어휘",
            "guide": "다문화 배경의 언어를 가르치는 강사.",
            "exampleKo": "다문화 언어 강사를 하고 싶어요.",
            "exampleZh": "我想做多元文化语言讲师。"
        },
        {
            "ko": "편의점 아르바이트",
            "zh": "便利店兼职",
            "pos": "명사구",
            "page": 132,
            "source": "어휘",
            "guide": "편의점에서 시간제로 하는 일.",
            "exampleKo": "편의점 아르바이트를 구했어요.",
            "exampleZh": "找到了便利店兼职。"
        },
        {
            "ko": "시간제로 할 수 있는 일",
            "zh": "可以按小时做的工作",
            "pos": "명사구",
            "page": 132,
            "source": "어휘",
            "guide": "정해진 시간만 일하는 형태의 일.",
            "exampleKo": "시간제로 할 수 있는 일을 찾고 있어요.",
            "exampleZh": "正在找可以按小时做的工作。"
        },
        {
            "ko": "사업하다",
            "zh": "做生意；创业",
            "pos": "동사",
            "page": 132,
            "source": "어휘",
            "guide": "자기 일을 운영하다.",
            "exampleKo": "나중에 사업하고 싶어요.",
            "exampleZh": "以后想做生意。"
        },
        {
            "ko": "가게를 차리다",
            "zh": "开店",
            "pos": "표현",
            "page": 132,
            "source": "어휘",
            "guide": "가게를 새로 열어 운영하다.",
            "exampleKo": "가게를 차리기 위해 준비하고 있어요.",
            "exampleZh": "为了开店正在准备。"
        },
        {
            "ko": "통역",
            "zh": "口译",
            "pos": "명사",
            "page": 132,
            "source": "어휘",
            "guide": "말을 다른 언어로 바꾸어 전달하는 일.",
            "exampleKo": "통역 일을 하고 싶어요.",
            "exampleZh": "我想做口译工作。"
        },
        {
            "ko": "번역",
            "zh": "翻译；笔译",
            "pos": "명사",
            "page": 132,
            "source": "어휘",
            "guide": "글을 다른 언어로 바꾸는 일.",
            "exampleKo": "번역 일을 배워 보고 싶어요.",
            "exampleZh": "我想学着做翻译工作。"
        },
        {
            "ko": "전문성 있는 일",
            "zh": "有专业性的工作",
            "pos": "명사구",
            "page": 132,
            "source": "어휘",
            "guide": "특정 지식이나 기술이 필요한 일.",
            "exampleKo": "전문성 있는 일을 하고 싶어요.",
            "exampleZh": "我想做有专业性的工作。"
        },
        {
            "ko": "안정적인 회사",
            "zh": "稳定的公司",
            "pos": "명사구",
            "page": 132,
            "source": "어휘",
            "guide": "일자리와 운영이 비교적 안정된 회사.",
            "exampleKo": "안정적인 회사에서 일하고 싶어요.",
            "exampleZh": "我想在稳定的公司工作。"
        },
        {
            "ko": "근무 환경",
            "zh": "工作环境",
            "pos": "명사구",
            "page": 132,
            "source": "어휘",
            "guide": "일하는 장소와 조건.",
            "exampleKo": "근무 환경이 좋은 회사를 원해요.",
            "exampleZh": "我想要工作环境好的公司。"
        },
        {
            "ko": "월급",
            "zh": "月薪",
            "pos": "명사",
            "page": 132,
            "source": "어휘",
            "guide": "한 달에 한 번 받는 급여.",
            "exampleKo": "월급을 많이 주는 회사가 좋아요.",
            "exampleZh": "喜欢给月薪高的公司。"
        },
        {
            "ko": "출퇴근이 자유롭다",
            "zh": "上下班自由",
            "pos": "표현",
            "page": 132,
            "source": "어휘",
            "guide": "출근과 퇴근 시간이 비교적 자유롭다.",
            "exampleKo": "출퇴근이 자유로운 회사에서 일하고 싶어요.",
            "exampleZh": "我想在上下班自由的公司工作。"
        },
        {
            "ko": "발전 가능성",
            "zh": "发展可能性",
            "pos": "명사구",
            "page": 132,
            "source": "어휘",
            "guide": "앞으로 더 좋아지거나 커질 가능성.",
            "exampleKo": "발전 가능성이 있는 회사를 찾고 있어요.",
            "exampleZh": "正在找有发展可能性的公司。"
        },
        {
            "ko": "사회에 기여하다",
            "zh": "为社会做贡献",
            "pos": "표현",
            "page": 132,
            "source": "어휘",
            "guide": "사회에 도움이 되는 일을 하다.",
            "exampleKo": "사회에 기여할 수 있는 일을 하고 싶어요.",
            "exampleZh": "我想做能为社会做贡献的事。"
        },
        {
            "ko": "자기 계발",
            "zh": "自我提升",
            "pos": "명사구",
            "page": 132,
            "source": "어휘",
            "guide": "자신의 능력을 키우는 일.",
            "exampleKo": "자기 계발을 할 수 있는 회사가 좋아요.",
            "exampleZh": "喜欢能自我提升的公司。"
        },
        {
            "ko": "보람을 느끼다",
            "zh": "感到有价值；有成就感",
            "pos": "표현",
            "page": 132,
            "source": "어휘",
            "guide": "일을 하며 의미와 만족을 느끼다.",
            "exampleKo": "일에 대한 보람을 느껴요.",
            "exampleZh": "对工作感到有成就感。"
        },
        {
            "ko": "구인 광고",
            "zh": "招聘广告",
            "pos": "명사구",
            "page": 132,
            "source": "어휘",
            "guide": "사람을 구한다는 광고.",
            "exampleKo": "구인 광고를 봤어요.",
            "exampleZh": "看了招聘广告。"
        },
        {
            "ko": "이력서",
            "zh": "简历",
            "pos": "명사",
            "page": 132,
            "source": "어휘",
            "guide": "학력과 경력 등을 적은 문서.",
            "exampleKo": "이력서를 써 놓았어요.",
            "exampleZh": "把简历提前写好了。"
        },
        {
            "ko": "지원서",
            "zh": "申请表；报名表",
            "pos": "명사",
            "page": 132,
            "source": "어휘",
            "guide": "회사나 기관에 지원하기 위해 쓰는 서류.",
            "exampleKo": "지원서를 작성했어요.",
            "exampleZh": "填写了申请表。"
        },
        {
            "ko": "작성하다",
            "zh": "填写；撰写",
            "pos": "동사",
            "page": 132,
            "source": "어휘",
            "guide": "문서나 글을 쓰다.",
            "exampleKo": "지원서를 작성해야 해요.",
            "exampleZh": "需要填写申请表。"
        },
        {
            "ko": "서류",
            "zh": "材料；文件",
            "pos": "명사",
            "page": 132,
            "source": "어휘",
            "guide": "제출하는 문서.",
            "exampleKo": "서류를 준비해 놓았어요.",
            "exampleZh": "把材料提前准备好了。"
        },
        {
            "ko": "제출하다",
            "zh": "提交",
            "pos": "동사",
            "page": 132,
            "source": "어휘",
            "guide": "문서나 과제를 내다.",
            "exampleKo": "서류를 제출했어요.",
            "exampleZh": "提交了材料。"
        },
        {
            "ko": "필기시험을 보다",
            "zh": "参加笔试",
            "pos": "표현",
            "page": 132,
            "source": "어휘",
            "guide": "글로 답하는 시험을 치르다.",
            "exampleKo": "필기시험을 봤어요.",
            "exampleZh": "参加了笔试。"
        },
        {
            "ko": "면접을 보다",
            "zh": "参加面试",
            "pos": "표현",
            "page": 132,
            "source": "어휘",
            "guide": "회사 사람을 만나 질문에 답하다.",
            "exampleKo": "내일 면접을 봐요.",
            "exampleZh": "明天参加面试。"
        },
        {
            "ko": "합격 통보 문자",
            "zh": "录取通知短信",
            "pos": "명사구",
            "page": 132,
            "source": "어휘",
            "guide": "합격했다는 내용을 알려 주는 문자.",
            "exampleKo": "합격 통보 문자를 받았어요.",
            "exampleZh": "收到了录取通知短信。"
        },
        {
            "ko": "준비하다",
            "zh": "准备",
            "pos": "동사",
            "page": 133,
            "source": "문법 연습",
            "guide": "필요한 것을 미리 갖추다.",
            "exampleKo": "무엇을 준비하고 있어요?",
            "exampleZh": "正在准备什么？"
        },
        {
            "ko": "자격증",
            "zh": "资格证",
            "pos": "명사",
            "page": 133,
            "source": "문법 연습",
            "guide": "일정한 자격을 증명하는 문서.",
            "exampleKo": "자격증을 따 놓으세요.",
            "exampleZh": "请提前考取资格证。"
        },
        {
            "ko": "컴퓨터 자격증",
            "zh": "电脑资格证",
            "pos": "명사구",
            "page": 133,
            "source": "문법 연습",
            "guide": "컴퓨터 능력을 증명하는 자격증.",
            "exampleKo": "컴퓨터 자격증을 미리 따 놓으세요.",
            "exampleZh": "请提前考取电脑资格证。"
        },
        {
            "ko": "미리",
            "zh": "提前",
            "pos": "부사",
            "page": 134,
            "source": "문법",
            "guide": "어떤 일이 있기 전에 먼저.",
            "exampleKo": "미리 예약해 놓았어요.",
            "exampleZh": "已经提前预约好了。"
        },
        {
            "ko": "따다",
            "zh": "取得；考取",
            "pos": "동사",
            "page": 134,
            "source": "말하기",
            "guide": "자격증이나 면허를 얻다.",
            "exampleKo": "자격증을 따고 싶어요.",
            "exampleZh": "想考取资格证。"
        },
        {
            "ko": "한국 국적",
            "zh": "韩国国籍",
            "pos": "명사구",
            "page": 133,
            "source": "예문",
            "guide": "한국 국민으로서의 자격.",
            "exampleKo": "한국 국적을 취득하기 위해 공부하고 있어요.",
            "exampleZh": "为了取得韩国国籍正在学习。"
        },
        {
            "ko": "취득하다",
            "zh": "取得",
            "pos": "동사",
            "page": 133,
            "source": "예문",
            "guide": "자격이나 권리를 얻다.",
            "exampleKo": "한국 국적을 취득하고 싶어요.",
            "exampleZh": "想取得韩国国籍。"
        },
        {
            "ko": "통역사",
            "zh": "口译员",
            "pos": "명사",
            "page": 133,
            "source": "문법 연습",
            "guide": "통역 일을 전문적으로 하는 사람.",
            "exampleKo": "통역사가 되기 위해 공부해요.",
            "exampleZh": "为了成为口译员而学习。"
        },
        {
            "ko": "일회용품",
            "zh": "一次性用品",
            "pos": "명사",
            "page": 133,
            "source": "예문",
            "guide": "한 번 쓰고 버리는 물건.",
            "exampleKo": "일회용품 사용을 줄여야 해요.",
            "exampleZh": "应该减少一次性用品使用。"
        },
        {
            "ko": "사용을 줄이다",
            "zh": "减少使用",
            "pos": "표현",
            "page": 133,
            "source": "예문",
            "guide": "쓰는 양이나 횟수를 낮추다.",
            "exampleKo": "쓰레기를 줄이기 위해 사용을 줄여요.",
            "exampleZh": "为了减少垃圾而减少使用。"
        },
        {
            "ko": "아이 맡길 곳",
            "zh": "托管孩子的地方",
            "pos": "명사구",
            "page": 133,
            "source": "예문",
            "guide": "아이를 돌봐 줄 수 있는 곳.",
            "exampleKo": "아이 맡길 곳을 찾고 있어요.",
            "exampleZh": "正在找托管孩子的地方。"
        },
        {
            "ko": "노력하다",
            "zh": "努力",
            "pos": "동사",
            "page": 133,
            "source": "예문",
            "guide": "목표를 이루기 위해 힘쓰다.",
            "exampleKo": "취업하기 위해 노력하고 있어요.",
            "exampleZh": "为了就业正在努力。"
        },
        {
            "ko": "정성껏 쓰다",
            "zh": "用心写",
            "pos": "표현",
            "page": 135,
            "source": "말하기",
            "guide": "마음을 다해 꼼꼼하게 쓰다.",
            "exampleKo": "이력서를 정성껏 쓰세요.",
            "exampleZh": "请用心写简历。"
        },
        {
            "ko": "무역 회사",
            "zh": "贸易公司",
            "pos": "명사구",
            "page": 135,
            "source": "말하기",
            "guide": "수출입과 거래를 하는 회사.",
            "exampleKo": "무역 회사에 취직하고 싶어요.",
            "exampleZh": "想进贸易公司工作。"
        },
        {
            "ko": "한국말",
            "zh": "韩语",
            "pos": "명사",
            "page": 135,
            "source": "말하기",
            "guide": "한국어를 일상적으로 이르는 말.",
            "exampleKo": "한국말을 어느 정도 할 수 있어요.",
            "exampleZh": "韩语会到一定程度。"
        },
        {
            "ko": "영어를 가르치다",
            "zh": "教英语",
            "pos": "표현",
            "page": 135,
            "source": "말하기 연습",
            "guide": "영어 수업을 하다.",
            "exampleKo": "학교에서 영어를 가르치고 싶어요.",
            "exampleZh": "想在学校教英语。"
        },
        {
            "ko": "지원하다",
            "zh": "应聘；申请",
            "pos": "동사",
            "page": 136,
            "source": "듣기",
            "guide": "회사나 학교에 들어가려고 신청하다.",
            "exampleKo": "우리 회사에 왜 지원했습니까?",
            "exampleZh": "为什么应聘我们公司？"
        },
        {
            "ko": "입사하다",
            "zh": "入职；进公司",
            "pos": "동사",
            "page": 136,
            "source": "듣기",
            "guide": "회사에 들어가 직원이 되다.",
            "exampleKo": "입사하면 어떤 일을 하고 싶습니까?",
            "exampleZh": "入职后想做什么工作？"
        },
        {
            "ko": "지원한 이유",
            "zh": "应聘理由",
            "pos": "명사구",
            "page": 136,
            "source": "듣기",
            "guide": "왜 지원했는지에 대한 이유.",
            "exampleKo": "지원한 이유가 무엇입니까?",
            "exampleZh": "应聘理由是什么？"
        },
        {
            "ko": "경력",
            "zh": "经历；工作经验",
            "pos": "명사",
            "page": 136,
            "source": "듣기/발음",
            "guide": "전에 일한 경험.",
            "exampleKo": "경력이 있습니까?",
            "exampleZh": "有工作经验吗？"
        },
        {
            "ko": "경력 증명서",
            "zh": "工作经历证明书",
            "pos": "명사구",
            "page": 136,
            "source": "발음",
            "guide": "전에 일한 사실을 증명하는 서류.",
            "exampleKo": "경력 증명서를 제출해 주세요.",
            "exampleZh": "请提交工作经历证明书。"
        },
        {
            "ko": "모집",
            "zh": "招募；招聘",
            "pos": "명사",
            "page": 137,
            "source": "읽기 전",
            "guide": "필요한 사람을 모으는 일.",
            "exampleKo": "신입 사원을 모집합니다.",
            "exampleZh": "招聘新员工。"
        },
        {
            "ko": "신입",
            "zh": "新人；应届/新入职",
            "pos": "명사",
            "page": 137,
            "source": "읽기 전",
            "guide": "새로 들어오는 사람.",
            "exampleKo": "신입 사원 모집 광고를 봤어요.",
            "exampleZh": "看了招聘新人职员的广告。"
        },
        {
            "ko": "경력 사원",
            "zh": "有经验职员",
            "pos": "명사구",
            "page": 137,
            "source": "읽기 전",
            "guide": "일한 경험이 있는 직원.",
            "exampleKo": "경력 사원을 모집합니다.",
            "exampleZh": "招聘有经验职员。"
        },
        {
            "ko": "모집 분야",
            "zh": "招聘领域/岗位",
            "pos": "명사구",
            "page": 137,
            "source": "읽기 전",
            "guide": "어떤 일을 할 사람을 뽑는지 나타내는 항목.",
            "exampleKo": "모집 분야는 판매직입니다.",
            "exampleZh": "招聘岗位是销售职。"
        },
        {
            "ko": "화장품 판매직",
            "zh": "化妆品销售岗位",
            "pos": "명사구",
            "page": 137,
            "source": "읽기 전",
            "guide": "화장품을 파는 일자리.",
            "exampleKo": "화장품 판매직에 지원했어요.",
            "exampleZh": "应聘了化妆品销售岗位。"
        },
        {
            "ko": "모집 인원",
            "zh": "招聘人数",
            "pos": "명사구",
            "page": 137,
            "source": "읽기 전",
            "guide": "뽑으려는 사람 수.",
            "exampleKo": "모집 인원은 00명입니다.",
            "exampleZh": "招聘人数是若干名。"
        },
        {
            "ko": "지원 자격",
            "zh": "应聘资格",
            "pos": "명사구",
            "page": 137,
            "source": "읽기 전",
            "guide": "지원할 수 있는 조건.",
            "exampleKo": "지원 자격을 확인하세요.",
            "exampleZh": "请确认应聘资格。"
        },
        {
            "ko": "고졸",
            "zh": "高中毕业",
            "pos": "명사",
            "page": 137,
            "source": "읽기 전",
            "guide": "고등학교를 졸업한 학력.",
            "exampleKo": "지원 자격은 고졸입니다.",
            "exampleZh": "应聘资格是高中毕业。"
        },
        {
            "ko": "문의",
            "zh": "咨询；问询",
            "pos": "명사",
            "page": 137,
            "source": "읽기 전",
            "guide": "궁금한 점을 물어보는 것.",
            "exampleKo": "문의 전화번호를 확인했어요.",
            "exampleZh": "确认了咨询电话号码。"
        },
        {
            "ko": "지원 방법",
            "zh": "应聘方式",
            "pos": "명사구",
            "page": 137,
            "source": "읽기 전",
            "guide": "어떻게 지원하는지에 대한 방법.",
            "exampleKo": "지원 방법은 온라인 접수입니다.",
            "exampleZh": "应聘方式是线上接收。"
        },
        {
            "ko": "온라인 접수",
            "zh": "线上申请/网上接收",
            "pos": "명사구",
            "page": 137,
            "source": "읽기 전",
            "guide": "인터넷으로 서류를 내는 방식.",
            "exampleKo": "온라인 접수를 했어요.",
            "exampleZh": "进行了线上申请。"
        },
        {
            "ko": "판매직",
            "zh": "销售岗位",
            "pos": "명사",
            "page": 137,
            "source": "단어장",
            "guide": "물건을 파는 직무.",
            "exampleKo": "판매직을 찾고 있어요.",
            "exampleZh": "正在找销售岗位。"
        },
        {
            "ko": "영업직",
            "zh": "营业/业务岗位",
            "pos": "명사",
            "page": 137,
            "source": "단어장",
            "guide": "고객을 만나 판매나 계약을 하는 직무.",
            "exampleKo": "영업직 경력이 있어요.",
            "exampleZh": "有业务岗位经验。"
        },
        {
            "ko": "생산직",
            "zh": "生产岗位",
            "pos": "명사",
            "page": 137,
            "source": "단어장",
            "guide": "공장에서 제품을 만드는 직무.",
            "exampleKo": "생산직 직원을 모집합니다.",
            "exampleZh": "招聘生产岗位员工。"
        },
        {
            "ko": "약간명",
            "zh": "若干名",
            "pos": "명사",
            "page": 137,
            "source": "단어장",
            "guide": "정확한 수를 밝히지 않고 조금 뽑는다는 뜻.",
            "exampleKo": "모집 인원은 약간명입니다.",
            "exampleZh": "招聘人数为若干名。"
        },
        {
            "ko": "대졸",
            "zh": "大学毕业",
            "pos": "명사",
            "page": 137,
            "source": "단어장",
            "guide": "대학교를 졸업한 학력.",
            "exampleKo": "지원 자격은 대졸입니다.",
            "exampleZh": "应聘资格是大学毕业。"
        },
        {
            "ko": "경력 2년 이상",
            "zh": "两年以上经验",
            "pos": "명사구",
            "page": 137,
            "source": "단어장",
            "guide": "일한 기간이 2년보다 길거나 같음.",
            "exampleKo": "경력 2년 이상인 사람을 찾습니다.",
            "exampleZh": "寻找两年以上经验的人。"
        },
        {
            "ko": "자격증 사본",
            "zh": "资格证复印件",
            "pos": "명사구",
            "page": 137,
            "source": "단어장",
            "guide": "자격증을 복사한 서류.",
            "exampleKo": "자격증 사본을 제출하세요.",
            "exampleZh": "请提交资格证复印件。"
        },
        {
            "ko": "운전면허증",
            "zh": "驾驶证",
            "pos": "명사",
            "page": 137,
            "source": "단어장",
            "guide": "운전할 수 있음을 증명하는 증명서.",
            "exampleKo": "운전면허증이 필요합니다.",
            "exampleZh": "需要驾驶证。"
        },
        {
            "ko": "방문 접수",
            "zh": "现场申请",
            "pos": "명사구",
            "page": 137,
            "source": "단어장",
            "guide": "직접 찾아가 서류를 내는 방식.",
            "exampleKo": "방문 접수도 가능합니다.",
            "exampleZh": "也可以现场申请。"
        },
        {
            "ko": "이메일",
            "zh": "电子邮件",
            "pos": "명사",
            "page": 137,
            "source": "단어장",
            "guide": "인터넷 편지 주소 또는 메시지.",
            "exampleKo": "이메일로 지원서를 보냈어요.",
            "exampleZh": "用电子邮件发送了申请表。"
        },
        {
            "ko": "자기소개서",
            "zh": "自我介绍书",
            "pos": "명사",
            "page": 137,
            "source": "제출 서류",
            "guide": "자신의 경험과 장점을 소개하는 글.",
            "exampleKo": "자기소개서를 써 놓았어요.",
            "exampleZh": "已经写好了自我介绍书。"
        },
        {
            "ko": "성적 증명서",
            "zh": "成绩证明书",
            "pos": "명사구",
            "page": 137,
            "source": "제출 서류",
            "guide": "학교 성적을 증명하는 서류.",
            "exampleKo": "성적 증명서를 냈어요.",
            "exampleZh": "提交了成绩证明书。"
        },
        {
            "ko": "졸업 증명서",
            "zh": "毕业证明书",
            "pos": "명사구",
            "page": 137,
            "source": "제출 서류",
            "guide": "학교를 졸업했음을 증명하는 서류.",
            "exampleKo": "졸업 증명서를 준비하세요.",
            "exampleZh": "请准备毕业证明书。"
        },
        {
            "ko": "가족 관계 증명서",
            "zh": "家庭关系证明书",
            "pos": "명사구",
            "page": 137,
            "source": "제출 서류",
            "guide": "가족 관계를 증명하는 서류.",
            "exampleKo": "가족 관계 증명서가 필요해요.",
            "exampleZh": "需要家庭关系证明书。"
        },
        {
            "ko": "여권 사본",
            "zh": "护照复印件",
            "pos": "명사구",
            "page": 137,
            "source": "제출 서류",
            "guide": "여권을 복사한 서류.",
            "exampleKo": "여권 사본을 제출하세요.",
            "exampleZh": "请提交护照复印件。"
        },
        {
            "ko": "외국인 등록증 사본",
            "zh": "外国人登记证复印件",
            "pos": "명사구",
            "page": 137,
            "source": "제출 서류",
            "guide": "외국인 등록증을 복사한 서류.",
            "exampleKo": "외국인 등록증 사본을 냈어요.",
            "exampleZh": "提交了外国人登记证复印件。"
        },
        {
            "ko": "방과 후 영어 교사",
            "zh": "课后英语教师",
            "pos": "명사구",
            "page": 138,
            "source": "읽기",
            "guide": "방과 후 수업에서 영어를 가르치는 교사.",
            "exampleKo": "방과 후 영어 교사를 모집합니다.",
            "exampleZh": "招聘课后英语教师。"
        },
        {
            "ko": "근무 시간",
            "zh": "工作时间",
            "pos": "명사구",
            "page": 138,
            "source": "읽기",
            "guide": "일하는 시간.",
            "exampleKo": "근무 시간은 월요일부터 금요일까지입니다.",
            "exampleZh": "工作时间是周一到周五。"
        },
        {
            "ko": "월~금",
            "zh": "周一到周五",
            "pos": "명사구",
            "page": 138,
            "source": "읽기",
            "guide": "월요일부터 금요일까지.",
            "exampleKo": "월~금 14시부터 19시까지 일합니다.",
            "exampleZh": "周一到周五14点到19点工作。"
        },
        {
            "ko": "문구점",
            "zh": "文具店",
            "pos": "명사",
            "page": 138,
            "source": "읽기",
            "guide": "문구류를 파는 가게.",
            "exampleKo": "문구점 직원을 구합니다.",
            "exampleZh": "招聘文具店员工。"
        },
        {
            "ko": "직원",
            "zh": "员工",
            "pos": "명사",
            "page": 138,
            "source": "읽기",
            "guide": "회사나 가게에서 일하는 사람.",
            "exampleKo": "직원을 구하고 있어요.",
            "exampleZh": "正在招聘员工。"
        },
        {
            "ko": "사무용품 배송",
            "zh": "办公用品配送",
            "pos": "명사구",
            "page": 138,
            "source": "읽기",
            "guide": "사무용품을 배달하는 일.",
            "exampleKo": "업무는 사무용품 배송입니다.",
            "exampleZh": "工作内容是办公用品配送。"
        },
        {
            "ko": "소지자",
            "zh": "持有者",
            "pos": "명사",
            "page": 138,
            "source": "읽기",
            "guide": "어떤 자격이나 물건을 가지고 있는 사람.",
            "exampleKo": "운전면허증 소지자를 찾습니다.",
            "exampleZh": "寻找驾驶证持有者。"
        },
        {
            "ko": "이름",
            "zh": "姓名",
            "pos": "명사",
            "page": 139,
            "source": "쓰기",
            "guide": "사람을 부르는 명칭.",
            "exampleKo": "이력서에 이름을 씁니다.",
            "exampleZh": "在简历上写姓名。"
        },
        {
            "ko": "생년월일",
            "zh": "出生年月日",
            "pos": "명사",
            "page": 139,
            "source": "쓰기",
            "guide": "태어난 해, 달, 날.",
            "exampleKo": "생년월일을 정확히 쓰세요.",
            "exampleZh": "请准确填写出生年月日。"
        },
        {
            "ko": "국적",
            "zh": "国籍",
            "pos": "명사",
            "page": 139,
            "source": "쓰기",
            "guide": "어느 나라 사람인지 나타내는 자격.",
            "exampleKo": "국적은 필수 항목입니다.",
            "exampleZh": "国籍是必填项目。"
        },
        {
            "ko": "전화번호",
            "zh": "电话号码",
            "pos": "명사",
            "page": 139,
            "source": "쓰기",
            "guide": "전화 연락을 위한 번호.",
            "exampleKo": "전화번호를 확인하세요.",
            "exampleZh": "请确认电话号码。"
        },
        {
            "ko": "휴대전화",
            "zh": "手机",
            "pos": "명사",
            "page": 139,
            "source": "쓰기",
            "guide": "가지고 다니는 전화기.",
            "exampleKo": "휴대전화 번호를 씁니다.",
            "exampleZh": "填写手机号码。"
        },
        {
            "ko": "여권번호",
            "zh": "护照号码",
            "pos": "명사",
            "page": 139,
            "source": "쓰기",
            "guide": "여권에 적힌 번호.",
            "exampleKo": "여권번호를 적어 주세요.",
            "exampleZh": "请填写护照号码。"
        },
        {
            "ko": "주소",
            "zh": "地址",
            "pos": "명사",
            "page": 139,
            "source": "쓰기",
            "guide": "사는 곳의 위치.",
            "exampleKo": "주소를 자세히 쓰세요.",
            "exampleZh": "请详细填写地址。"
        },
        {
            "ko": "학력",
            "zh": "学历",
            "pos": "명사",
            "page": 139,
            "source": "쓰기",
            "guide": "학교를 다닌 이력.",
            "exampleKo": "학력 사항을 작성합니다.",
            "exampleZh": "填写学历事项。"
        },
        {
            "ko": "기간",
            "zh": "期间",
            "pos": "명사",
            "page": 139,
            "source": "쓰기",
            "guide": "어떤 일이 계속된 시간.",
            "exampleKo": "재학 기간을 씁니다.",
            "exampleZh": "填写在学期间。"
        },
        {
            "ko": "학교명",
            "zh": "学校名称",
            "pos": "명사",
            "page": 139,
            "source": "쓰기",
            "guide": "학교의 이름.",
            "exampleKo": "학교명을 정확히 쓰세요.",
            "exampleZh": "请准确填写学校名称。"
        },
        {
            "ko": "전공",
            "zh": "专业",
            "pos": "명사",
            "page": 139,
            "source": "쓰기",
            "guide": "주로 공부한 분야.",
            "exampleKo": "전공은 영어입니다.",
            "exampleZh": "专业是英语。"
        },
        {
            "ko": "구분",
            "zh": "区分；类别",
            "pos": "명사",
            "page": 139,
            "source": "쓰기",
            "guide": "종류를 나누는 항목.",
            "exampleKo": "졸업 여부를 구분에 씁니다.",
            "exampleZh": "在类别栏填写是否毕业。"
        },
        {
            "ko": "기관명",
            "zh": "机构名称",
            "pos": "명사",
            "page": 139,
            "source": "쓰기",
            "guide": "회사나 기관의 이름.",
            "exampleKo": "경력란에 기관명을 적습니다.",
            "exampleZh": "在经历栏填写机构名称。"
        },
        {
            "ko": "직위",
            "zh": "职位",
            "pos": "명사",
            "page": 139,
            "source": "쓰기",
            "guide": "회사에서 맡은 자리.",
            "exampleKo": "직위는 사원입니다.",
            "exampleZh": "职位是职员。"
        },
        {
            "ko": "비고",
            "zh": "备注",
            "pos": "명사",
            "page": 139,
            "source": "쓰기",
            "guide": "덧붙여 쓰는 내용.",
            "exampleKo": "비고란에 추가 내용을 씁니다.",
            "exampleZh": "在备注栏写补充内容。"
        },
        {
            "ko": "취득일",
            "zh": "取得日期",
            "pos": "명사",
            "page": 139,
            "source": "쓰기",
            "guide": "자격증 등을 얻은 날짜.",
            "exampleKo": "자격증 취득일을 쓰세요.",
            "exampleZh": "请填写资格证取得日期。"
        },
        {
            "ko": "등급",
            "zh": "等级",
            "pos": "명사",
            "page": 139,
            "source": "쓰기",
            "guide": "수준을 나눈 단계.",
            "exampleKo": "자격증 등급을 확인하세요.",
            "exampleZh": "请确认资格证等级。"
        },
        {
            "ko": "발행처",
            "zh": "发行机构",
            "pos": "명사",
            "page": 139,
            "source": "쓰기",
            "guide": "증명서나 자격증을 발행한 곳.",
            "exampleKo": "발행처를 써야 해요.",
            "exampleZh": "需要填写发行机构。"
        },
        {
            "ko": "기재하다",
            "zh": "记载；填写",
            "pos": "동사",
            "page": 139,
            "source": "쓰기",
            "guide": "문서에 내용을 적다.",
            "exampleKo": "기재한 사항은 사실입니다.",
            "exampleZh": "记载事项属实。"
        },
        {
            "ko": "성명",
            "zh": "姓名",
            "pos": "명사",
            "page": 139,
            "source": "쓰기",
            "guide": "성과 이름.",
            "exampleKo": "마지막에 성명을 씁니다.",
            "exampleZh": "最后写姓名。"
        },
        {
            "ko": "급여일",
            "zh": "发薪日",
            "pos": "명사",
            "page": 140,
            "source": "문화와 정보",
            "guide": "급여를 받는 날.",
            "exampleKo": "직장인들이 기다리는 날은 급여일입니다.",
            "exampleZh": "职场人等待的日子是发薪日。"
        },
        {
            "ko": "급여",
            "zh": "工资；薪酬",
            "pos": "명사",
            "page": 140,
            "source": "문화와 정보",
            "guide": "일하고 받는 돈.",
            "exampleKo": "급여는 은행 계좌로 받습니다.",
            "exampleZh": "工资通过银行账户领取。"
        },
        {
            "ko": "기본급",
            "zh": "基本工资",
            "pos": "명사",
            "page": 140,
            "source": "문화와 정보",
            "guide": "일하고 받는 기본적인 급여.",
            "exampleKo": "급여에는 기본급이 있습니다.",
            "exampleZh": "工资里有基本工资。"
        },
        {
            "ko": "수당",
            "zh": "津贴；补贴",
            "pos": "명사",
            "page": 140,
            "source": "문화와 정보",
            "guide": "기본급 외에 추가로 받는 돈.",
            "exampleKo": "수당의 종류가 다양합니다.",
            "exampleZh": "津贴种类很多。"
        },
        {
            "ko": "가족 수당",
            "zh": "家庭津贴",
            "pos": "명사구",
            "page": 140,
            "source": "문화와 정보",
            "guide": "가족 상황에 따라 받는 수당.",
            "exampleKo": "가족 수당을 받을 수 있어요.",
            "exampleZh": "可以领取家庭津贴。"
        },
        {
            "ko": "초과 근무 수당",
            "zh": "加班津贴",
            "pos": "명사구",
            "page": 140,
            "source": "문화와 정보",
            "guide": "정해진 시간보다 더 일하고 받는 수당.",
            "exampleKo": "초과 근무 수당이 나왔어요.",
            "exampleZh": "发了加班津贴。"
        },
        {
            "ko": "상여 수당",
            "zh": "奖金津贴",
            "pos": "명사구",
            "page": 140,
            "source": "문화와 정보",
            "guide": "상여금 성격으로 받는 수당.",
            "exampleKo": "상여 수당을 받았어요.",
            "exampleZh": "收到了奖金津贴。"
        },
        {
            "ko": "은행 계좌",
            "zh": "银行账户",
            "pos": "명사구",
            "page": 140,
            "source": "문화와 정보",
            "guide": "은행에 만든 돈 입출금 계정.",
            "exampleKo": "급여가 은행 계좌로 입금됩니다.",
            "exampleZh": "工资打进银行账户。"
        },
        {
            "ko": "입금되다",
            "zh": "入账",
            "pos": "동사",
            "page": 140,
            "source": "문화와 정보",
            "guide": "돈이 계좌에 들어오다.",
            "exampleKo": "월급이 계좌에 입금되었어요.",
            "exampleZh": "工资入账了。"
        },
        {
            "ko": "지급되다",
            "zh": "被支付；发放",
            "pos": "동사",
            "page": 140,
            "source": "문화와 정보",
            "guide": "돈이나 물건이 주어지다.",
            "exampleKo": "실제 지급되는 월급과 차이가 있어요.",
            "exampleZh": "和实际发放的月薪有差异。"
        },
        {
            "ko": "세금",
            "zh": "税金",
            "pos": "명사",
            "page": 140,
            "source": "문화와 정보",
            "guide": "국가나 지방자치단체에 내는 돈.",
            "exampleKo": "급여에서 세금이 공제됩니다.",
            "exampleZh": "工资中会扣除税金。"
        },
        {
            "ko": "건강 보험료",
            "zh": "健康保险费",
            "pos": "명사구",
            "page": 140,
            "source": "문화와 정보",
            "guide": "건강보험을 위해 내는 돈.",
            "exampleKo": "건강 보험료도 급여에서 공제됩니다.",
            "exampleZh": "健康保险费也从工资中扣除。"
        },
        {
            "ko": "정기 적금",
            "zh": "定期储蓄",
            "pos": "명사구",
            "page": 140,
            "source": "문화와 정보",
            "guide": "정해진 기간 동안 일정액을 저축하는 것.",
            "exampleKo": "정기 적금이 급여에서 빠져나가요.",
            "exampleZh": "定期储蓄会从工资中扣出。"
        },
        {
            "ko": "공제하다",
            "zh": "扣除",
            "pos": "동사",
            "page": 140,
            "source": "문화와 정보",
            "guide": "전체 금액에서 빼다.",
            "exampleKo": "세금을 급여에서 공제합니다.",
            "exampleZh": "从工资中扣除税金。"
        },
        {
            "ko": "액수",
            "zh": "金额",
            "pos": "명사",
            "page": 140,
            "source": "문화와 정보",
            "guide": "돈의 양.",
            "exampleKo": "급여 액수에 따라 세금이 달라져요.",
            "exampleZh": "税金会根据工资金额变化。"
        },
        {
            "ko": "비율",
            "zh": "比例",
            "pos": "명사",
            "page": 140,
            "source": "문화와 정보",
            "guide": "전체에서 차지하는 정도.",
            "exampleKo": "세금을 내는 비율이 높아집니다.",
            "exampleZh": "缴税比例会提高。"
        },
        {
            "ko": "분야",
            "zh": "领域；岗位类别",
            "pos": "명사",
            "page": 141,
            "source": "배운 어휘 확인",
            "guide": "일이나 공부의 범위.",
            "exampleKo": "어떤 분야에 지원했어요?",
            "exampleZh": "应聘了什么领域？"
        },
        {
            "ko": "인원",
            "zh": "人员；人数",
            "pos": "명사",
            "page": 141,
            "source": "배운 어휘 확인",
            "guide": "사람 수.",
            "exampleKo": "모집 인원을 확인하세요.",
            "exampleZh": "请确认招聘人数。"
        },
        {
            "ko": "증명서",
            "zh": "证明书",
            "pos": "명사",
            "page": 141,
            "source": "배운 어휘 확인",
            "guide": "어떤 사실을 증명하는 서류.",
            "exampleKo": "증명서를 제출해야 해요.",
            "exampleZh": "需要提交证明书。"
        },
        {
            "ko": "사본",
            "zh": "复印件",
            "pos": "명사",
            "page": 141,
            "source": "배운 어휘 확인",
            "guide": "원본을 복사한 것.",
            "exampleKo": "신분증 사본을 준비하세요.",
            "exampleZh": "请准备身份证件复印件。"
        },
        {
            "ko": "전문성",
            "zh": "专业性",
            "pos": "명사",
            "page": 141,
            "source": "배운 어휘 확인",
            "guide": "전문 지식이나 능력이 있는 성질.",
            "exampleKo": "전문성이 있는 일을 하고 싶어요.",
            "exampleZh": "想做有专业性的工作。"
        }
    ],
    "dialogues": [
        {
            "id": "l3-10-d1",
            "title": "취업 준비 목적 말하기",
            "page": 133,
            "source": "문법 1 대화",
            "focus": "동-기 위해서",
            "sceneZh": "朋友询问为了进入韩国公司工作正在准备什么。",
            "lines": [
                {
                    "speaker": "잠시드",
                    "ko": "한국 회사에 취직하기 위해서 뭘 준비하고 있어요?",
                    "zh": "为了进入韩国公司工作，你正在准备什么？",
                    "guide": "취직하기 위해서 明确说明“准备”的目的。"
                },
                {
                    "speaker": "고천",
                    "ko": "저는 컴퓨터를 배우고 있어요.",
                    "zh": "我正在学电脑。",
                    "guide": "목적句后面常接 구체적인 준비。"
                }
            ],
            "learningPoints": [
                "동사词干后接 -기 위해서 表示目的。",
                "求职话题中可替换为 자격증을 따기 위해서, 면접을 잘 보기 위해서。"
            ],
            "drills": [
                {
                    "promptZh": "说“为了成为口译员，我在学习韩语”。",
                    "pattern": "-기 위해서",
                    "answerKo": "통역사가 되기 위해서 한국어를 공부하고 있어요."
                },
                {
                    "promptZh": "说“为了取得韩国国籍，从一年前开始学习韩语”。",
                    "pattern": "-기 위해서",
                    "answerKo": "한국 국적을 취득하기 위해서 1년 전부터 한국어를 공부하고 있어요."
                }
            ]
        },
        {
            "id": "l3-10-d2",
            "title": "미리 해 놓은 일 말하기",
            "page": 134,
            "source": "문법 2 대화",
            "focus": "동-어 놓다",
            "sceneZh": "假期前确认机票是否已经提前预约好。",
            "lines": [
                {
                    "speaker": "후엔",
                    "ko": "휴가철인데 비행기표가 있을까요?",
                    "zh": "是假期，不知道还有没有机票呢？",
                    "guide": "휴가철 表示休假旺季，票可能紧张。"
                },
                {
                    "speaker": "박민수",
                    "ko": "걱정 말아요. 미리 예약해 놓았어요.",
                    "zh": "别担心。我已经提前预约好了。",
                    "guide": "예약해 놓았어요 表示已经预约，结果现在仍有效。"
                }
            ],
            "learningPoints": [
                "-어 놓다 强调动作完成后的结果保持。",
                "미리 和 -어 놓다 常一起使用，表示提前准备好。"
            ],
            "drills": [
                {
                    "promptZh": "说“已经提前准备好礼物了”。",
                    "pattern": "-어 놓다",
                    "answerKo": "선물을 미리 준비해 놓았어요."
                },
                {
                    "promptZh": "说“提交材料前已经把申请表写好了”。",
                    "pattern": "-어 놓다",
                    "answerKo": "서류를 제출하기 전에 지원서를 써 놓았어요."
                }
            ]
        },
        {
            "id": "l3-10-d3",
            "title": "한국에서 일하기 위해 필요한 것 묻기",
            "page": 135,
            "source": "말하기",
            "focus": "취업 조언",
            "sceneZh": "아나이스 想进贸易公司，라흐만 建议她准备简历和电脑资格证。",
            "lines": [
                {
                    "speaker": "라흐만",
                    "ko": "아나이스 씨는 무슨 일을 하고 싶어요?",
                    "zh": "Anais，你想做什么工作？",
                    "guide": "무슨 일을 하고 싶어요? 是询问职业愿望的自然说法。"
                },
                {
                    "speaker": "아나이스",
                    "ko": "저는 무역 회사에 취직하고 싶어요.",
                    "zh": "我想进贸易公司工作。",
                    "guide": "회사에 취직하다 表示进入某公司就职。"
                },
                {
                    "speaker": "라흐만",
                    "ko": "아, 그러세요? 아나이스 씨는 한국말을 어느 정도 할 수 있으니까 잘할 수 있을 거예요.",
                    "zh": "啊，是吗？Anais 你的韩语会到一定程度，应该能做得好。",
                    "guide": "어느 정도 할 수 있다 表示“能到一定程度”。"
                },
                {
                    "speaker": "아나이스",
                    "ko": "그런데 한국에서 일하기 위해서 무엇이 필요해요?",
                    "zh": "不过，为了在韩国工作，需要什么呢？",
                    "guide": "일하기 위해서 把问题聚焦到“为了工作”。"
                },
                {
                    "speaker": "라흐만",
                    "ko": "무역 회사에 취직하려면 컴퓨터를 잘해야 해요. 그러니까 이력서를 정성껏 쓰고 컴퓨터 자격증도 미리 따 놓으세요.",
                    "zh": "如果想进贸易公司，就要会电脑。所以请用心写简历，也提前考好电脑资格证。",
                    "guide": "-려면 提出条件，-아 놓으세요 表示建议提前做好。"
                },
                {
                    "speaker": "아나이스",
                    "ko": "네, 그럴게요.",
                    "zh": "好的，我会那样做。",
                    "guide": "그럴게요 表示接受建议并承诺行动。"
                }
            ],
            "learningPoints": [
                "求职建议可以按“目标工作 -> 必要能力 -> 提前准备”组织。",
                "정성껏 쓰다, 미리 따 놓다 是简历和资格证建议里的高频搭配。"
            ],
            "drills": [
                {
                    "promptZh": "把目标换成“在学校教英语”，并建议准备面试。",
                    "pattern": "면접시험을 준비해 놓다",
                    "answerKo": "학교에서 영어를 가르치려면 면접을 잘 봐야 해요. 그러니까 면접시험을 미리 준비해 놓으세요."
                },
                {
                    "promptZh": "说“为了就业，我正在提前准备简历”。",
                    "pattern": "-기 위해서 / -어 놓다",
                    "answerKo": "취업하기 위해서 이력서를 미리 써 놓고 있어요."
                }
            ]
        }
    ],
    "readings": [
        {
            "title": "외국인 영어 강사를 모십니다",
            "page": 138,
            "source": "읽기 1",
            "paragraphs": [
                {
                    "ko": "외국인 영어 강사를 모십니다. 모집 분야는 방과 후 영어 교사이고, 지원 자격은 대졸(영어 전공)입니다. 제출 서류는 이력서와 여권 사본입니다.",
                    "zh": "招聘外籍英语讲师。招聘岗位是课后英语教师，应聘资格是大学毕业（英语专业）。提交材料是简历和护照复印件。",
                    "guide": "招聘广告先看 모집 분야, 지원 자격, 제출 서류。"
                },
                {
                    "ko": "근무 시간은 월요일부터 금요일까지 오후 2시부터 7시까지입니다. 문의는 학교 전화로 할 수 있습니다.",
                    "zh": "工作时间是周一到周五下午2点到7点。可以通过学校电话咨询。",
                    "guide": "근무 시간과 문의처는 지원前一定要确认。"
                }
            ],
            "questions": [
                {
                    "q": "이 광고는 어떤 사람을 모집하는 광고입니까?",
                    "zh": "这则广告招聘什么人？",
                    "a": "방과 후 영어 교사를 모집하는 광고입니다."
                },
                {
                    "q": "어떤 서류를 제출해야 합니까?",
                    "zh": "需要提交哪些材料？",
                    "a": "이력서와 여권 사본을 제출해야 합니다."
                },
                {
                    "q": "누가 지원할 수 있습니까?",
                    "zh": "谁可以应聘？",
                    "a": "영어 전공 대졸자가 지원할 수 있습니다."
                }
            ]
        },
        {
            "title": "문구점 직원 구함",
            "page": 138,
            "source": "읽기 2",
            "paragraphs": [
                {
                    "ko": "문구점 직원을 구합니다. 업무는 사무용품 배송이고, 지원 자격은 고졸과 운전면허증 소지자입니다.",
                    "zh": "招聘文具店员工。工作内容是办公用品配送，应聘资格是高中毕业并持有驾驶证。",
                    "guide": "업무와 자격을 연결해서 읽으면 지원 가능 여부를 판단하기 쉽다。"
                },
                {
                    "ko": "제출 서류는 이력서이고, 지원 방법은 온라인 접수입니다. 이메일로 지원할 수 있습니다.",
                    "zh": "提交材料是简历，应聘方式是网上申请。可以通过电子邮件申请。",
                    "guide": "온라인 접수는 방문하지 않고 인터넷으로 제출하는 방식。"
                }
            ],
            "questions": [
                {
                    "q": "문구점 직원은 어떤 일을 합니까?",
                    "zh": "文具店员工做什么工作？",
                    "a": "사무용품 배송을 합니다."
                },
                {
                    "q": "지원하려면 무엇이 필요합니까?",
                    "zh": "应聘需要什么？",
                    "a": "고졸 학력과 운전면허증이 필요합니다."
                },
                {
                    "q": "어떻게 지원합니까?",
                    "zh": "怎样应聘？",
                    "a": "이력서를 이메일로 보내 온라인 접수합니다."
                }
            ]
        }
    ],
    "culture": {
        "titleKo": "급여와 세금",
        "titleZh": "工资和税金",
        "page": 140,
        "source": "문화와 정보",
        "summaryZh": "这篇文化文章说明韩国职场人通常每月领一次工资，工资由基本工资和各种津贴构成；实际入账金额会因为税金、健康保险费、定期储蓄等扣除项目而少于名义工资。",
        "paragraphs": [
            {
                "ko": "직장인들이라면 누구나 기다리는 날이 있다. 바로 급여일이다. 한국의 직장인은 보통 한 달에 한 번씩 급여를 받는다. 그래서 한 달에 한 번 받는 것을 월급을 받는다고 한다. 급여는 크게 기본급과 수당이 있다. 기본급은 일을 하고 받는 기본적인 급여이다. 그리고 수당은 가족 수당, 초과 근무 수당, 상여 수당 등으로 종류가 다양하다.",
                "zh": "如果是职场人，都会有一个等待的日子，那就是发薪日。韩国的职场人通常一个月领一次工资，所以把一个月领一次叫作领月薪。工资大体上包括基本工资和津贴。基本工资是工作后获得的基本薪酬，津贴则有家庭津贴、加班津贴、奖金津贴等多种类型。",
                "guide": "第一段说明 월급 的领取周期，以及 급여 = 기본급 + 수당 的结构。"
            },
            {
                "ko": "한국의 직장인은 급여를 어떻게 받을까? 보통 한 달에 한 번씩 은행 계좌를 통해 받는다. 그런데 계좌에 입금되는 금액은 실제 지급되는 월급과 차이가 있다. 세금, 건강 보험료, 정기 적금 등을 급여에서 공제하기 때문이다.",
                "zh": "韩国职场人怎样领取工资呢？通常每个月通过银行账户领取一次。不过，打到账户里的金额和实际发放的月薪会有差异，因为税金、健康保险费、定期储蓄等会从工资中扣除。",
                "guide": "입금되는 금액 是到账金额，지급되는 월급 是发放工资，差异来自 공제。"
            },
            {
                "ko": "세금과 건강 보험료는 급여 액수에 따라 달라지는데 당연히 급여가 많으면 많을수록 세금과 건강 보험료도 많이 내게 된다. 단순히 세금의 액수가 많아지는 것이 아니고 급여 액수가 많을수록 세금을 내는 비율이 높아진다는 사실을 알 필요가 있다.",
                "zh": "税金和健康保险费会根据工资金额而变化，当然工资越高，缴纳的税金和健康保险费也越多。需要知道的是，不只是税金金额会变多，工资金额越高，缴税比例也会提高。",
                "guide": "-으면 -을수록 表示“越……越……”，这里解释累进税率的感觉。"
            }
        ],
        "keyTerms": [
            {
                "ko": "급여",
                "zh": "劳动后收到的工资或薪酬。"
            },
            {
                "ko": "기본급",
                "zh": "工资中的基本部分。"
            },
            {
                "ko": "수당",
                "zh": "基本工资之外的各种补贴。"
            },
            {
                "ko": "공제",
                "zh": "从工资里扣除税金、保险费等项目。"
            },
            {
                "ko": "세금",
                "zh": "依法缴纳给国家或地方政府的钱。"
            }
        ],
        "questions": [
            {
                "q": "한국의 직장인이 받는 급여는 어떻게 구성됩니까?",
                "zh": "韩国职场人领取的工资由什么构成？",
                "a": "크게 기본급과 수당으로 구성됩니다."
            },
            {
                "q": "지급되는 월급과 계좌에 입금되는 금액이 다른 이유는 무엇입니까?",
                "zh": "发放工资和账户入账金额不同的原因是什么？",
                "a": "세금, 건강 보험료, 정기 적금 등이 급여에서 공제되기 때문입니다."
            },
            {
                "q": "급여 액수가 많아지면 세금 비율은 어떻게 됩니까?",
                "zh": "工资金额变高时，缴税比例会怎样？",
                "a": "세금을 내는 비율도 높아집니다."
            }
        ]
    },
    "practice": [
        {
            "type": "grammar",
            "title": "-기 위해서 목적 말하기",
            "prompt": "“为了进入贸易公司，我正在学习电脑”를 한국어로 말해 보세요.",
            "answer": "무역 회사에 취직하기 위해서 컴퓨터를 배우고 있어요."
        },
        {
            "type": "grammar",
            "title": "-어 놓다로 준비 말하기",
            "prompt": "“请提前写好简历”를 한국어로 말해 보세요.",
            "answer": "이력서를 미리 써 놓으세요."
        },
        {
            "type": "speaking",
            "title": "취업 조언하기",
            "prompt": "무역 회사에 취직하고 싶은 친구에게 준비할 것을 말해 보세요.",
            "answer": "컴퓨터를 잘해야 해요. 이력서를 정성껏 쓰고 컴퓨터 자격증도 미리 따 놓으세요."
        },
        {
            "type": "vocabulary",
            "title": "구인 광고 읽기",
            "prompt": "모집 분야, 지원 자격, 제출 서류, 지원 방법의 중국어 뜻을 말해 보세요.",
            "answer": "招聘岗位、应聘资格、提交材料、应聘方式。"
        },
        {
            "type": "reading",
            "title": "지원 가능 여부 판단하기",
            "prompt": "문구점 배송 직원에 지원하려면 어떤 자격이 필요합니까?",
            "answer": "고졸이고 운전면허증을 가지고 있어야 합니다."
        },
        {
            "type": "culture",
            "title": "월급과 실수령액",
            "prompt": "계좌에 입금되는 금액이 월급과 다른 이유를 설명하세요.",
            "answer": "세금, 건강 보험료, 정기 적금 등이 급여에서 공제되기 때문입니다."
        },
        {
            "type": "writing",
            "title": "이력서 항목 쓰기",
            "prompt": "이력서에 들어가는 항목 다섯 개를 한국어로 쓰세요.",
            "answer": "이름, 생년월일, 국적, 전화번호, 학력."
        }
    ],
    "translationGuide": [
        {
            "title": "-기 위해서 和 “为了”",
            "tip": "中文可直接译成“为了……”，但韩语后面要接具体行动：취직하기 위해서 준비하다, 합격하기 위해서 공부하다。"
        },
        {
            "title": "-어 놓다 的“结果保留”",
            "tip": "它不是单纯过去式，而是“提前做完，并且这个准备现在还有效”。이력서를 써 놓다 就是简历已经写好可使用。"
        },
        {
            "title": "招聘广告先抓四项",
            "tip": "读招聘广告时先找 모집 분야, 지원 자격, 제출 서류, 지원 방법，再看 근무 시간 和 문의。"
        },
        {
            "title": "급여 和 월급",
            "tip": "급여 是较正式的“薪酬/工资”，월급 强调每月领一次的钱。实际到账金额要考虑 공제。"
        }
    ]
};
