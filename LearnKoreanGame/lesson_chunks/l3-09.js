window.lessonGuideL3LessonChunks ||= {};
window.lessonGuideL3LessonChunks["l3-09"] = {
    "id": "l3-09",
    "number": 9,
    "titleKo": "고장과 수리",
    "titleZh": "故障和修理",
    "pages": "118-129",
    "status": "draft",
    "sourceFile": "kiip-3-textbook.pdf",
    "progress": {
        "vocabulary": "audited-p118-p129",
        "dialogue": "reviewed-draft",
        "culture": "reviewed-draft"
    },
    "theme": "学习说明家庭和电子产品故障，向服务中心咨询，并理解保修期和免费维修条件。",
    "goals": [
        "能说出家里或电子产品出现了什么故障。",
        "能用 동/형-아서 그런지 推测原因。",
        "能用 동-나요?, 형-은가요? 礼貌柔和地提问。",
        "能给服务中心打电话说明问题并请求维修。",
        "能阅读保修期说明，判断是否能免费维修。"
    ],
    "vocabularySources": [
        {
            "page": 118,
            "label": "단원 목표",
            "note": "单元主题、语法、活动和文化标题。"
        },
        {
            "page": 120,
            "label": "어휘",
            "note": "家庭故障、电子产品故障和服务中心处理流程。"
        },
        {
            "page": 121,
            "label": "문법: -아서 그런지",
            "note": "推测故障原因。"
        },
        {
            "page": 122,
            "label": "문법: -나요?, -은가요?",
            "note": "礼貌提问和维修咨询。"
        },
        {
            "page": 123,
            "label": "말하기",
            "note": "服务中心电话咨询对话。"
        },
        {
            "page": 124,
            "label": "듣기/발음",
            "note": "手机维修、保险、客户要求和发音词。"
        },
        {
            "page": 125,
            "label": "읽기 전",
            "note": "故障前自查步骤和常见问答。"
        },
        {
            "page": 126,
            "label": "읽기",
            "note": "网络게시판 변기 수리问答。"
        },
        {
            "page": 128,
            "label": "문화와 정보",
            "note": "전자 제품 보증 기간文化文章词汇。"
        },
        {
            "page": 129,
            "label": "배운 어휘 확인",
            "note": "补充复习词表中的故障、维修和保修词。"
        }
    ],
    "grammar": [
        {
            "pattern": "동/형-아서 그런지",
            "zh": "也许是因为……；可能因为……",
            "page": 121,
            "guide": "用于原因不确定时的推测说明。前面说可能的原因，后面说结果。中文常译为“也许是因为……所以……”。",
            "examples": [
                {
                    "ko": "딱딱한 걸 계속 씹어서 그런지 이가 아프네요.",
                    "zh": "也许是因为一直咬硬的东西，牙疼呢。"
                },
                {
                    "ko": "지하에 있어서 그런지 와이파이 연결이 잘 안 돼요.",
                    "zh": "可能因为在地下，Wi-Fi连接不太好。"
                },
                {
                    "ko": "냉동실에 음식물을 많이 넣어서 그런지 냉동이 잘 안 돼요.",
                    "zh": "也许是因为冷冻室里放了很多食物，冷冻不太好。"
                }
            ]
        },
        {
            "pattern": "동-나요?, 형-은가요?",
            "zh": "柔和提问：……吗？",
            "page": 122,
            "guide": "询问对方时比普通疑问更柔和、亲切。维修咨询里常用 해야 되나요?, 걸리나요?, 비싼가요?, 가능한가요?。",
            "examples": [
                {
                    "ko": "예약을 해야 되나요?",
                    "zh": "需要预约吗？"
                },
                {
                    "ko": "고치는 데 얼마나 걸리나요?",
                    "zh": "修理需要多长时间？"
                },
                {
                    "ko": "수리 비용이 많이 비싼가요?",
                    "zh": "修理费用很贵吗？"
                }
            ]
        }
    ],
    "vocabulary": [
        {
            "ko": "고장과 수리",
            "zh": "故障和修理",
            "pos": "명사구",
            "page": 118,
            "source": "단원 제목",
            "guide": "固定表达，意思是“故障和修理”。建议按整块记忆，写作和口语里可以直接套用。",
            "exampleKo": "9과의 주제는 고장과 수리입니다.",
            "exampleZh": "第9课主题是故障和修理。"
        },
        {
            "ko": "고장",
            "zh": "故障",
            "pos": "명사",
            "page": 118,
            "source": "단원 제목",
            "guide": "名词，意思是“故障”。可以作为句子的主语或宾语来使用，复习时注意它常出现的生活场景。",
            "exampleKo": "노트북에 고장이 났어요.",
            "exampleZh": "笔记本电脑出故障了。"
        },
        {
            "ko": "수리",
            "zh": "修理",
            "pos": "명사",
            "page": 118,
            "source": "단원 제목",
            "guide": "名词，意思是“修理”。可以作为句子的主语或宾语来使用，复习时注意它常出现的生活场景。",
            "exampleKo": "수리를 받아야 해요.",
            "exampleZh": "需要修理。"
        },
        {
            "ko": "서비스 센터",
            "zh": "服务中心",
            "pos": "명사",
            "page": 118,
            "source": "활동 목표",
            "guide": "名词，意思是“服务中心”。可以作为句子的主语或宾语来使用，复习时注意它常出现的生活场景。",
            "exampleKo": "서비스 센터에 문의하세요.",
            "exampleZh": "请咨询服务中心。"
        },
        {
            "ko": "전화 문의",
            "zh": "电话咨询",
            "pos": "명사구",
            "page": 118,
            "source": "활동 목표",
            "guide": "固定表达，意思是“电话咨询”。建议按整块记忆，写作和口语里可以直接套用。",
            "tips": [
                { "label": "搭配", "text": "전화 문의를 드리다" }
            ],
            "exampleKo": "서비스 센터에 전화 문의를 했어요.",
            "exampleZh": "给服务中心打电话咨询了。"
        },
        {
            "ko": "집 수리 요청",
            "zh": "请求房屋维修",
            "pos": "명사구",
            "page": 118,
            "source": "활동 목표",
            "guide": "固定表达，意思是“请求房屋维修”。建议按整块记忆，写作和口语里可以直接套用。",
            "exampleKo": "집 수리 요청하는 글을 썼어요.",
            "exampleZh": "写了请求房屋维修的文字。"
        },
        {
            "ko": "전자 제품 보증 기간",
            "zh": "电子产品保修期",
            "pos": "명사구",
            "page": 118,
            "source": "문화 제목",
            "guide": "固定表达，意思是“电子产品保修期”。建议按整块记忆，写作和口语里可以直接套用。",
            "tips": [
                { "label": "搭配", "text": "보증 기간을 확인하다 / 지나다" }
            ],
            "exampleKo": "전자 제품 보증 기간을 확인하세요.",
            "exampleZh": "请确认电子产品保修期。"
        },
        {
            "ko": "문제가 생기다",
            "zh": "出现问题",
            "pos": "표현",
            "page": 119,
            "source": "도입",
            "guide": "固定表达，意思是“出现问题”。建议按整块记忆，写作和口语里可以直接套用。",
            "tips": [
                { "label": "易错", "text": "생기다=出现；있다=已有" }
            ],
            "exampleKo": "무슨 문제가 생긴 것 같아요?",
            "exampleZh": "好像出了什么问题？"
        },
        {
            "ko": "하수구",
            "zh": "下水口",
            "pos": "명사",
            "page": 120,
            "source": "어휘",
            "guide": "名词，意思是“下水口”。可以作为句子的主语或宾语来使用，复习时注意它常出现的生活场景。",
            "exampleKo": "하수구에 머리카락이 들어갔어요.",
            "exampleZh": "头发进了下水口。"
        },
        {
            "ko": "하수구가 막히다",
            "zh": "下水口堵了",
            "pos": "표현",
            "page": 120,
            "source": "어휘",
            "guide": "固定表达，意思是“下水口堵了”。建议按整块记忆，写作和口语里可以直接套用。",
            "tips": [
                { "label": "易错", "text": "막히다=被堵，막다=堵住" }
            ],
            "exampleKo": "하수구가 막혀서 물이 안 내려가요.",
            "exampleZh": "下水口堵了，水下不去。"
        },
        {
            "ko": "변기",
            "zh": "马桶",
            "pos": "명사",
            "page": 120,
            "source": "어휘",
            "guide": "名词，意思是“马桶”。可以作为句子的主语或宾语来使用，复习时注意它常出现的生活场景。",
            "exampleKo": "변기에 문제가 있어요.",
            "exampleZh": "马桶有问题。"
        },
        {
            "ko": "변기가 막히다",
            "zh": "马桶堵了",
            "pos": "표현",
            "page": 120,
            "source": "어휘",
            "guide": "固定表达，意思是“马桶堵了”。建议按整块记忆，写作和口语里可以直接套用。",
            "exampleKo": "변기가 자주 막혀요.",
            "exampleZh": "马桶经常堵。"
        },
        {
            "ko": "물이 안 나오다",
            "zh": "不出水",
            "pos": "표현",
            "page": 120,
            "source": "어휘",
            "guide": "固定表达，意思是“不出水”。建议按整块记忆，写作和口语里可以直接套用。",
            "tips": [
                { "label": "搭配", "text": "물/화면/소리가 안 나오다" }
            ],
            "exampleKo": "세면대에서 물이 안 나와요.",
            "exampleZh": "洗手台不出水。"
        },
        {
            "ko": "물이 새다",
            "zh": "漏水",
            "pos": "표현",
            "page": 120,
            "source": "어휘",
            "guide": "固定表达，意思是“漏水”。建议按整块记忆，写作和口语里可以直接套用。",
            "tips": [
                { "label": "易错", "text": "새다=漏；세다=强/数数" }
            ],
            "exampleKo": "천장에서 물이 새요.",
            "exampleZh": "天花板漏水。"
        },
        {
            "ko": "전등",
            "zh": "电灯",
            "pos": "명사",
            "page": 120,
            "source": "어휘",
            "guide": "名词，意思是“电灯”。可以作为句子的主语或宾语来使用，复习时注意它常出现的生活场景。",
            "exampleKo": "전등이 갑자기 꺼졌어요.",
            "exampleZh": "电灯突然灭了。"
        },
        {
            "ko": "전등이 나가다",
            "zh": "灯坏了/灯灭了",
            "pos": "표현",
            "page": 120,
            "source": "어휘",
            "guide": "固定表达，意思是“灯坏了/灯灭了”。建议按整块记忆，写作和口语里可以直接套用。",
            "tips": [
                { "label": "搭配", "text": "전등이 나가다 / 갈다" },
                { "label": "易错", "text": "灯坏了自然说나가다" }
            ],
            "exampleKo": "거실 전등이 나갔어요.",
            "exampleZh": "客厅灯坏了。"
        },
        {
            "ko": "문이 잠기다",
            "zh": "门锁上/打不开",
            "pos": "표현",
            "page": 120,
            "source": "어휘",
            "guide": "固定表达，意思是“门锁上/打不开”。建议按整块记忆，写作和口语里可以直接套用。",
            "tips": [
                { "label": "易错", "text": "잠그다=锁上；잠기다=锁着" }
            ],
            "exampleKo": "문이 잠겨서 못 들어가요.",
            "exampleZh": "门锁住了进不去。"
        },
        {
            "ko": "가스불이 안 들어오다",
            "zh": "燃气点不着",
            "pos": "표현",
            "page": 120,
            "source": "어휘",
            "guide": "固定表达，意思是“燃气点不着”。建议按整块记忆，写作和口语里可以直接套用。",
            "tips": [
                { "label": "搭配", "text": "가스불이 들어오다 / 안 들어오다" }
            ],
            "exampleKo": "가스불이 안 들어와요.",
            "exampleZh": "燃气点不着。"
        },
        {
            "ko": "냉동이 안 되다",
            "zh": "不能冷冻",
            "pos": "표현",
            "page": 120,
            "source": "어휘",
            "guide": "固定表达，意思是“不能冷冻”。建议按整块记忆，写作和口语里可以直接套用。",
            "tips": [
                { "label": "易错", "text": "功能不工作常说안 되다" }
            ],
            "exampleKo": "냉장고 냉동이 안 돼요.",
            "exampleZh": "冰箱不能冷冻。"
        },
        {
            "ko": "와이파이 연결이 안 되다",
            "zh": "Wi-Fi连不上",
            "pos": "표현",
            "page": 120,
            "source": "어휘",
            "guide": "固定表达，意思是“Wi-Fi连不上”。建议按整块记忆，写作和口语里可以直接套用。",
            "tips": [
                { "label": "搭配", "text": "연결이 안 되다 / 끊기다" }
            ],
            "exampleKo": "와이파이 연결이 안 돼요.",
            "exampleZh": "Wi-Fi连不上。"
        },
        {
            "ko": "액정이 깨지다",
            "zh": "屏幕碎了",
            "pos": "표현",
            "page": 120,
            "source": "어휘",
            "guide": "固定表达，意思是“屏幕碎了”。建议按整块记忆，写作和口语里可以直接套用。",
            "tips": [
                { "label": "搭配", "text": "액정이 깨지다 / 금이 가다" }
            ],
            "exampleKo": "휴대 전화 액정이 깨졌어요.",
            "exampleZh": "手机屏幕碎了。"
        },
        {
            "ko": "부팅이 안 되다",
            "zh": "无法启动",
            "pos": "표현",
            "page": 120,
            "source": "어휘",
            "guide": "固定表达，意思是“无法启动”。建议按整块记忆，写作和口语里可以直接套用。",
            "tips": [
                { "label": "搭配", "text": "부팅이 안 되다" }
            ],
            "exampleKo": "노트북 부팅이 안 돼요.",
            "exampleZh": "笔记本无法启动。"
        },
        {
            "ko": "전원이 안 켜지다",
            "zh": "电源打不开",
            "pos": "표현",
            "page": 120,
            "source": "어휘",
            "guide": "固定表达，意思是“电源打不开”。建议按整块记忆，写作和口语里可以直接套用。",
            "tips": [
                { "label": "易错", "text": "켜다主动，켜지다状态/被动" }
            ],
            "exampleKo": "어제부터 전원이 안 켜져요.",
            "exampleZh": "从昨天开始电源打不开。"
        },
        {
            "ko": "고장이 나다",
            "zh": "出故障",
            "pos": "표현",
            "page": 120,
            "source": "어휘/배운 어휘 확인",
            "guide": "固定表达，意思是“出故障”。建议按整块记忆，写作和口语里可以直接套用。",
            "tips": [
                { "label": "搭配", "text": "전자 제품에 고장이 나다" }
            ],
            "exampleKo": "전자 제품이 고장 났어요.",
            "exampleZh": "电子产品出故障了。"
        },
        {
            "ko": "문의하다",
            "zh": "咨询",
            "pos": "동사",
            "page": 120,
            "source": "어휘",
            "guide": "动词表达，意思是“咨询”。复习时把前面的宾语或场景一起记，方便在口语中直接使用。",
            "tips": [
                { "label": "搭配", "text": "서비스 센터에 문의하다" },
                { "label": "易错", "text": "比물어보다更正式" }
            ],
            "exampleKo": "서비스 센터에 문의하세요.",
            "exampleZh": "请咨询服务中心。"
        },
        {
            "ko": "출장 서비스",
            "zh": "上门服务",
            "pos": "명사구",
            "page": 120,
            "source": "어휘",
            "guide": "固定表达，意思是“上门服务”。建议按整块记忆，写作和口语里可以直接套用。",
            "tips": [
                { "label": "搭配", "text": "출장 서비스를 신청하다" }
            ],
            "exampleKo": "출장 서비스를 신청했어요.",
            "exampleZh": "申请了上门服务。"
        },
        {
            "ko": "신청하다",
            "zh": "申请",
            "pos": "동사",
            "page": 120,
            "source": "어휘",
            "guide": "动词表达，意思是“申请”。复习时把前面的宾语或场景一起记，方便在口语中直接使用。",
            "exampleKo": "출장 서비스를 신청해야 해요.",
            "exampleZh": "需要申请上门服务。"
        },
        {
            "ko": "방문하다",
            "zh": "访问；到访",
            "pos": "동사",
            "page": 120,
            "source": "어휘",
            "guide": "动词表达，意思是“访问、到访”。复习时把前面的宾语或场景一起记，方便在口语中直接使用。",
            "exampleKo": "서비스 센터에 방문했어요.",
            "exampleZh": "去了服务中心。"
        },
        {
            "ko": "고치다",
            "zh": "修；修好",
            "pos": "동사",
            "page": 120,
            "source": "어휘",
            "guide": "动词表达，意思是“修、修好”。复习时把前面的宾语或场景一起记，方便在口语中直接使用。",
            "tips": [
                { "label": "易错", "text": "고치다较口语，수리하다较正式" }
            ],
            "exampleKo": "노트북을 고쳤어요.",
            "exampleZh": "把笔记本修好了。"
        },
        {
            "ko": "수리하다",
            "zh": "修理",
            "pos": "동사",
            "page": 120,
            "source": "어휘/배운 어휘 확인",
            "guide": "动词表达，意思是“修理”。复习时把前面的宾语或场景一起记，方便在口语中直接使用。",
            "exampleKo": "휴대 전화를 수리했어요.",
            "exampleZh": "修理了手机。"
        },
        {
            "ko": "수리 비용을 내다",
            "zh": "支付维修费",
            "pos": "표현",
            "page": 120,
            "source": "어휘",
            "guide": "固定表达，意思是“支付维修费”。建议按整块记忆，写作和口语里可以直接套用。",
            "tips": [
                { "label": "搭配", "text": "비용을 내다 / 지불하다" }
            ],
            "exampleKo": "고객이 수리 비용을 내야 합니다.",
            "exampleZh": "顾客需要支付维修费。"
        },
        {
            "ko": "무상 수리를 받다",
            "zh": "接受免费维修",
            "pos": "표현",
            "page": 120,
            "source": "어휘",
            "guide": "固定表达，意思是“接受免费维修”。建议按整块记忆，写作和口语里可以直接套用。",
            "tips": [
                { "label": "搭配", "text": "무상 수리를 받을 수 있다" }
            ],
            "exampleKo": "보증 기간이면 무상 수리를 받을 수 있어요.",
            "exampleZh": "在保修期内可以免费维修。"
        },
        {
            "ko": "이유를 추측하다",
            "zh": "推测原因",
            "pos": "동사구",
            "page": 121,
            "source": "문법",
            "guide": "动词表达，意思是“推测原因”。复习时把前面的宾语或场景一起记，方便在口语中直接使用。",
            "exampleKo": "고장 난 이유를 추측해 보세요.",
            "exampleZh": "请推测故障原因。"
        },
        {
            "ko": "딱딱하다",
            "zh": "硬",
            "pos": "형용사",
            "page": 121,
            "source": "문법 예문/배운 어휘 확인",
            "guide": "形容词，意思是“硬”。主要用来描述状态、性质或感受，适合和人物、物品、情况一起造句。",
            "exampleKo": "딱딱한 걸 씹어서 이가 아파요.",
            "exampleZh": "咬硬的东西所以牙疼。"
        },
        {
            "ko": "씹다",
            "zh": "咀嚼",
            "pos": "동사",
            "page": 121,
            "source": "문법 예문",
            "guide": "动词表达，意思是“咀嚼”。复习时把前面的宾语或场景一起记，方便在口语中直接使用。",
            "exampleKo": "딱딱한 것을 계속 씹었어요.",
            "exampleZh": "一直咬硬的东西。"
        },
        {
            "ko": "이가 아프다",
            "zh": "牙疼",
            "pos": "표현",
            "page": 121,
            "source": "문법 예문",
            "guide": "固定表达，意思是“牙疼”。建议按整块记忆，写作和口语里可以直接套用。",
            "exampleKo": "이가 아파서 병원에 갔어요.",
            "exampleZh": "牙疼所以去了医院。"
        },
        {
            "ko": "아르바이트",
            "zh": "兼职",
            "pos": "명사",
            "page": 121,
            "source": "문법 예문",
            "guide": "名词，意思是“兼职”。可以作为句子的主语或宾语来使用，复习时注意它常出现的生活场景。",
            "exampleKo": "동생이 아르바이트를 시작했어요.",
            "exampleZh": "弟弟/妹妹开始兼职了。"
        },
        {
            "ko": "지하",
            "zh": "地下",
            "pos": "명사",
            "page": 121,
            "source": "문법 대화",
            "guide": "名词，意思是“地下”。可以作为句子的主语或宾语来使用，复习时注意它常出现的生活场景。",
            "exampleKo": "지하에 있어서 와이파이가 잘 안 돼요.",
            "exampleZh": "因为在地下，Wi-Fi不太好。"
        },
        {
            "ko": "머리카락",
            "zh": "头发",
            "pos": "명사",
            "page": 121,
            "source": "문법 연습",
            "guide": "名词，意思是“头发”。可以作为句子的主语或宾语来使用，复习时注意它常出现的生活场景。",
            "exampleKo": "머리카락이 들어가서 하수구가 막혔어요.",
            "exampleZh": "头发进去所以下水口堵了。"
        },
        {
            "ko": "물이 내려가다",
            "zh": "水流下去",
            "pos": "표현",
            "page": 121,
            "source": "문법 연습",
            "guide": "固定表达，意思是“水流下去”。建议按整块记忆，写作和口语里可以直接套用。",
            "exampleKo": "물이 잘 안 내려가요.",
            "exampleZh": "水不太下去。"
        },
        {
            "ko": "냉동실",
            "zh": "冷冻室",
            "pos": "명사",
            "page": 121,
            "source": "문법 연습",
            "guide": "名词，意思是“冷冻室”。可以作为句子的主语或宾语来使用，复习时注意它常出现的生活场景。",
            "exampleKo": "냉동실에 음식물을 많이 넣었어요.",
            "exampleZh": "冷冻室放了很多食物。"
        },
        {
            "ko": "음식물",
            "zh": "食物；食物残渣",
            "pos": "명사",
            "page": 121,
            "source": "문법 연습",
            "guide": "名词，意思是“食物、食物残渣”。可以作为句子的主语或宾语来使用，复习时注意它常出现的生活场景。",
            "exampleKo": "음식물을 너무 많이 넣지 마세요.",
            "exampleZh": "不要放太多食物。"
        },
        {
            "ko": "바닥",
            "zh": "地面",
            "pos": "명사",
            "page": 121,
            "source": "문법 연습/배운 어휘 확인",
            "guide": "名词，意思是“地面”。可以作为句子的主语或宾语来使用，复习时注意它常出现的生活场景。",
            "exampleKo": "휴대 전화를 바닥에 떨어뜨렸어요.",
            "exampleZh": "把手机掉到地上了。"
        },
        {
            "ko": "떨어뜨리다",
            "zh": "弄掉；掉落",
            "pos": "동사",
            "page": 121,
            "source": "문법 연습/배운 어휘 확인",
            "guide": "动词表达，意思是“弄掉、掉落”。复习时把前面的宾语或场景一起记，方便在口语中直接使用。",
            "tips": [
                { "label": "搭配", "text": "바닥에 떨어뜨리다" },
                { "label": "易错", "text": "떨어지다=掉下；떨어뜨리다=弄掉" }
            ],
            "exampleKo": "휴대 전화를 떨어뜨려서 액정이 깨졌어요.",
            "exampleZh": "手机掉了所以屏幕碎了。"
        },
        {
            "ko": "다운로드 받은 파일",
            "zh": "下载的文件",
            "pos": "명사구",
            "page": 121,
            "source": "문법 연습",
            "guide": "固定表达，意思是“下载的文件”。建议按整块记忆，写作和口语里可以直接套用。",
            "exampleKo": "다운로드 받은 파일이 많아서 속도가 느려요.",
            "exampleZh": "下载的文件多所以速度慢。"
        },
        {
            "ko": "속도",
            "zh": "速度",
            "pos": "명사",
            "page": 121,
            "source": "문법 연습/배운 어휘 확인",
            "guide": "名词，意思是“速度”。可以作为句子的主语或宾语来使用，复习时注意它常出现的生活场景。",
            "exampleKo": "휴대 전화 속도가 느려졌어요.",
            "exampleZh": "手机速度变慢了。"
        },
        {
            "ko": "느리다",
            "zh": "慢",
            "pos": "형용사",
            "page": 121,
            "source": "문법 연습",
            "guide": "形容词，意思是“慢”。主要用来描述状态、性质或感受，适合和人物、物品、情况一起造句。",
            "tips": [
                { "label": "搭配", "text": "속도가 느리다 / 빠르다" }
            ],
            "exampleKo": "인터넷이 너무 느려요.",
            "exampleZh": "网速太慢。"
        },
        {
            "ko": "부드럽다",
            "zh": "柔和；柔软",
            "pos": "형용사",
            "page": 122,
            "source": "문법 설명",
            "guide": "形容词，意思是“柔和、柔软”。主要用来描述状态、性质或感受，适合和人物、物品、情况一起造句。",
            "exampleKo": "-나요?는 부드러운 질문입니다.",
            "exampleZh": "“是不是……呢？”是比较柔和的提问方式。"
        },
        {
            "ko": "친근하다",
            "zh": "亲切；亲近",
            "pos": "형용사",
            "page": 122,
            "source": "문법 설명",
            "guide": "形容词，意思是“亲切、亲近”。主要用来描述状态、性质或感受，适合和人物、物品、情况一起造句。",
            "exampleKo": "친근한 느낌으로 질문할 수 있어요.",
            "exampleZh": "可以用亲切的感觉提问。"
        },
        {
            "ko": "고치는 데 얼마나 걸리다",
            "zh": "修理要多久",
            "pos": "표현",
            "page": 122,
            "source": "문법 예문",
            "guide": "固定表达，意思是“修理要多久”。建议按整块记忆，写作和口语里可以直接套用。",
            "tips": [
                { "label": "搭配", "text": "V-는 데 얼마나 걸리다" }
            ],
            "exampleKo": "고치는 데 얼마나 걸리나요?",
            "exampleZh": "修理要多久？"
        },
        {
            "ko": "이틀 정도",
            "zh": "两天左右",
            "pos": "명사구",
            "page": 122,
            "source": "문법 예문",
            "guide": "固定表达，意思是“两天左右”。建议按整块记忆，写作和口语里可以直接套用。",
            "exampleKo": "이틀 정도 걸립니다.",
            "exampleZh": "需要两天左右。"
        },
        {
            "ko": "예방 주사",
            "zh": "预防针",
            "pos": "명사",
            "page": 122,
            "source": "문법 예문/배운 어휘 확인",
            "guide": "名词，意思是“预防针”。可以作为句子的主语或宾语来使用，复习时注意它常出现的生活场景。",
            "exampleKo": "예방 주사 맞을 때 많이 아픈가요?",
            "exampleZh": "打预防针时很疼吗？"
        },
        {
            "ko": "예약하다",
            "zh": "预约",
            "pos": "동사",
            "page": 122,
            "source": "문법 대화",
            "guide": "动词表达，意思是“预约”。复习时把前面的宾语或场景一起记，方便在口语中直接使用。",
            "tips": [
                { "label": "搭配", "text": "예약을 하다 / 예약하고 가다" }
            ],
            "exampleKo": "서비스 센터에 예약하고 가세요.",
            "exampleZh": "请预约后去服务中心。"
        },
        {
            "ko": "점검하다",
            "zh": "检查；检修",
            "pos": "동사",
            "page": 122,
            "source": "문법 연습/배운 어휘 확인",
            "guide": "动词表达，意思是“检查、检修”。复习时把前面的宾语或场景一起记，方便在口语中直接使用。",
            "tips": [
                { "label": "搭配", "text": "기계를 점검하다 / 점검을 받다" }
            ],
            "exampleKo": "온도를 조절하고 점검해야 합니다.",
            "exampleZh": "需要调节温度并检查。"
        },
        {
            "ko": "출장비",
            "zh": "上门费",
            "pos": "명사",
            "page": 122,
            "source": "문법 연습",
            "guide": "名词，意思是“上门费”。可以作为句子的主语或宾语来使用，复习时注意它常出现的生活场景。",
            "tips": [
                { "label": "易错", "text": "출장비≠수리비" }
            ],
            "exampleKo": "수리 비용은 출장비만 나옵니다.",
            "exampleZh": "维修费用只产生上门费。"
        },
        {
            "ko": "고객님",
            "zh": "顾客（敬称）",
            "pos": "명사",
            "page": 123,
            "source": "말하기",
            "guide": "名词，意思是“顾客（敬称）”。可以作为句子的主语或宾语来使用，复习时注意它常出现的生活场景。",
            "tips": [
                { "label": "易错", "text": "님是敬称，自己不用" }
            ],
            "exampleKo": "네, 고객님. 어떤 문제가 있으세요?",
            "exampleZh": "好的，顾客，请问有什么问题？"
        },
        {
            "ko": "서비스 문의",
            "zh": "服务咨询",
            "pos": "명사구",
            "page": 123,
            "source": "말하기",
            "guide": "固定表达，意思是“服务咨询”。建议按整块记忆，写作和口语里可以直接套用。",
            "tips": [
                { "label": "搭配", "text": "서비스 문의 좀 드리다" }
            ],
            "exampleKo": "서비스 문의 좀 드리려고 하는데요.",
            "exampleZh": "我想咨询一下服务。"
        },
        {
            "ko": "말씀해 주시겠어요?",
            "zh": "能请您说明吗？",
            "pos": "표현",
            "page": 123,
            "source": "말하기",
            "guide": "固定表达，意思是“能请您说明吗？”。建议按整块记忆，写作和口语里可以直接套用。",
            "tips": [
                { "label": "易错", "text": "말씀은 말하다的尊敬表达" }
            ],
            "exampleKo": "어떤 문제가 있는지 말씀해 주시겠어요?",
            "exampleZh": "能请您说明有什么问题吗？"
        },
        {
            "ko": "갑자기",
            "zh": "突然",
            "pos": "부사",
            "page": 123,
            "source": "말하기",
            "guide": "副词，意思是“突然”。常用来修饰动作、程度或整句话的语气。",
            "exampleKo": "갑자기 문제가 생겼어요.",
            "exampleZh": "突然出现问题了。"
        },
        {
            "ko": "며칠 전부터",
            "zh": "从几天前开始",
            "pos": "표현",
            "page": 123,
            "source": "말하기",
            "guide": "固定表达，意思是“从几天前开始”。建议按整块记忆，写作和口语里可以直接套用。",
            "tips": [
                { "label": "搭配", "text": "며칠 전부터 계속" }
            ],
            "exampleKo": "며칠 전부터 속도가 느려졌어요.",
            "exampleZh": "从几天前开始速度变慢了。"
        },
        {
            "ko": "가까운 서비스 센터",
            "zh": "附近的服务中心",
            "pos": "명사구",
            "page": 123,
            "source": "말하기",
            "guide": "固定表达，意思是“附近的服务中心”。建议按整块记忆，写作和口语里可以直接套用。",
            "exampleKo": "가까운 서비스 센터에 방문하세요.",
            "exampleZh": "请去附近的服务中心。"
        },
        {
            "ko": "점검을 받다",
            "zh": "接受检查",
            "pos": "표현",
            "page": 123,
            "source": "말하기",
            "guide": "固定表达，意思是“接受检查”。建议按整块记忆，写作和口语里可以直接套用。",
            "tips": [
                { "label": "搭配", "text": "서비스 센터에서 점검을 받다" }
            ],
            "exampleKo": "서비스 센터에서 점검을 받아 보세요.",
            "exampleZh": "请在服务中心接受检查。"
        },
        {
            "ko": "예약을 부탁드리다",
            "zh": "拜托预约",
            "pos": "표현",
            "page": 123,
            "source": "말하기",
            "guide": "固定表达，意思是“拜托预约”。建议按整块记忆，写作和口语里可以直接套用。",
            "tips": [
                { "label": "易错", "text": "드리다是주다的谦让表达" }
            ],
            "exampleKo": "그럼 예약 좀 부탁드려요.",
            "exampleZh": "那麻烦帮我预约一下。"
        },
        {
            "ko": "텔레비전",
            "zh": "电视",
            "pos": "명사",
            "page": 123,
            "source": "말하기",
            "guide": "名词，意思是“电视”。可以作为句子的主语或宾语来使用，复习时注意它常出现的生活场景。",
            "exampleKo": "텔레비전 전원이 안 켜져요.",
            "exampleZh": "电视电源打不开。"
        },
        {
            "ko": "냉장고",
            "zh": "冰箱",
            "pos": "명사",
            "page": 123,
            "source": "말하기/발음",
            "guide": "名词，意思是“冰箱”。可以作为句子的主语或宾语来使用，复习时注意它常出现的生活场景。",
            "exampleKo": "냉장고가 고장 나면 출장 서비스를 신청하세요.",
            "exampleZh": "冰箱坏了的话请申请上门服务。"
        },
        {
            "ko": "냉기",
            "zh": "冷气；冷却力",
            "pos": "명사",
            "page": 123,
            "source": "문법 연습/배운 어휘 확인",
            "guide": "名词，意思是“冷气、冷却力”。可以作为句子的主语或宾语来使用，复习时注意它常出现的生活场景。",
            "tips": [
                { "label": "搭配", "text": "냉기가 약하다 / 나오다" }
            ],
            "exampleKo": "냉기가 약합니다.",
            "exampleZh": "冷气不足。"
        },
        {
            "ko": "휴대 전화",
            "zh": "手机",
            "pos": "명사",
            "page": 124,
            "source": "듣기",
            "guide": "名词，意思是“手机”。可以作为句子的主语或宾语来使用，复习时注意它常出现的生活场景。",
            "exampleKo": "휴대 전화 액정이 깨졌어요.",
            "exampleZh": "手机屏幕碎了。"
        },
        {
            "ko": "물이 들어가다",
            "zh": "进水",
            "pos": "표현",
            "page": 124,
            "source": "듣기",
            "guide": "固定表达，意思是“进水”。建议按整块记忆，写作和口语里可以直接套用。",
            "tips": [
                { "label": "搭配", "text": "휴대 전화에 물이 들어가다" }
            ],
            "exampleKo": "휴대 전화에 물이 들어갔어요.",
            "exampleZh": "手机进水了。"
        },
        {
            "ko": "보험에 가입하다",
            "zh": "加入保险",
            "pos": "표현",
            "page": 124,
            "source": "듣기/배운 어휘 확인",
            "guide": "固定表达，意思是“加入保险”。建议按整块记忆，写作和口语里可以直接套用。",
            "tips": [
                { "label": "搭配", "text": "보험에 가입하다 / 가입되어 있다" }
            ],
            "exampleKo": "휴대 전화 보험에 가입했어요.",
            "exampleZh": "加入了手机保险。"
        },
        {
            "ko": "제외하다",
            "zh": "除外",
            "pos": "동사",
            "page": 124,
            "source": "듣기/배운 어휘 확인",
            "guide": "动词表达，意思是“除外”。复习时把前面的宾语或场景一起记，方便在口语中直接使用。",
            "tips": [
                { "label": "易错", "text": "제외하다主动，제외되다被排除" }
            ],
            "exampleKo": "보험 대상에서 제외될 수 있어요.",
            "exampleZh": "可能被排除在保险对象之外。"
        },
        {
            "ko": "보험사",
            "zh": "保险公司",
            "pos": "명사",
            "page": 124,
            "source": "듣기",
            "guide": "名词，意思是“保险公司”。可以作为句子的主语或宾语来使用，复习时注意它常出现的生活场景。",
            "exampleKo": "서류를 보험사로 보내야 합니다.",
            "exampleZh": "要把文件寄给保险公司。"
        },
        {
            "ko": "일시적",
            "zh": "临时的；一时的",
            "pos": "명사/형용사",
            "page": 124,
            "source": "발음/배운 어휘 확인",
            "guide": "形容词，意思是“临时的、一时的”。主要用来描述状态、性质或感受，适合和人物、物品、情况一起造句。",
            "exampleKo": "와이파이 문제는 일시적일 수 있어요.",
            "exampleZh": "Wi-Fi问题可能是一时的。"
        },
        {
            "ko": "갈등",
            "zh": "矛盾；冲突",
            "pos": "명사",
            "page": 124,
            "source": "발음",
            "guide": "名词，意思是“矛盾、冲突”。可以作为句子的主语或宾语来使用，复习时注意它常出现的生活场景。",
            "exampleKo": "고객의 무리한 요구 때문에 갈등이 생겨요.",
            "exampleZh": "因为顾客的无理要求产生冲突。"
        },
        {
            "ko": "무리한 요구",
            "zh": "无理要求",
            "pos": "명사구",
            "page": 124,
            "source": "발음",
            "guide": "固定表达，意思是“无理要求”。建议按整块记忆，写作和口语里可以直接套用。",
            "tips": [
                { "label": "易错", "text": "무리하다=勉强/过分" }
            ],
            "exampleKo": "무리한 요구는 갈등을 만들 수 있어요.",
            "exampleZh": "无理要求可能造成冲突。"
        },
        {
            "ko": "플러그",
            "zh": "插头",
            "pos": "명사",
            "page": 125,
            "source": "읽기 전/배운 어휘 확인",
            "guide": "名词，意思是“插头”。可以作为句子的主语或宾语来使用，复习时注意它常出现的生活场景。",
            "exampleKo": "플러그를 잘 꽂았는지 확인하세요.",
            "exampleZh": "请确认插头是否插好了。"
        },
        {
            "ko": "뽑다",
            "zh": "拔出",
            "pos": "동사",
            "page": 125,
            "source": "읽기 전/배운 어휘 확인",
            "guide": "动词表达，意思是“拔出”。复习时把前面的宾语或场景一起记，方便在口语中直接使用。",
            "tips": [
                { "label": "易错", "text": "플러그는 뽑다，앱은 종료하다" }
            ],
            "exampleKo": "플러그를 뽑았다 다시 꽂아 보세요.",
            "exampleZh": "拔掉插头再插上试试。"
        },
        {
            "ko": "꽂다",
            "zh": "插入",
            "pos": "동사",
            "page": 125,
            "source": "읽기 전/배운 어휘 확인",
            "guide": "动词表达，意思是“插入”。复习时把前面的宾语或场景一起记，方便在口语中直接使用。",
            "tips": [
                { "label": "搭配", "text": "플러그를 꽂다" },
                { "label": "变形", "text": "꽂아요 / 꽂았어요" }
            ],
            "exampleKo": "플러그를 다시 꽂으세요.",
            "exampleZh": "请重新插上插头。"
        },
        {
            "ko": "밸브",
            "zh": "阀门",
            "pos": "명사",
            "page": 125,
            "source": "읽기 전/배운 어휘 확인",
            "guide": "名词，意思是“阀门”。可以作为句子的主语或宾语来使用，复习时注意它常出现的生活场景。",
            "exampleKo": "밸브를 열었는지 확인하세요.",
            "exampleZh": "请确认阀门是否打开。"
        },
        {
            "ko": "잠그다",
            "zh": "锁上；关紧",
            "pos": "동사",
            "page": 125,
            "source": "읽기 전",
            "guide": "动词表达，意思是“锁上、关紧”。复习时把前面的宾语或场景一起记，方便在口语中直接使用。",
            "tips": [
                { "label": "搭配", "text": "밸브/문을 잠그다" }
            ],
            "exampleKo": "가스 밸브를 잠그세요.",
            "exampleZh": "请关紧燃气阀门。"
        },
        {
            "ko": "열다",
            "zh": "打开",
            "pos": "동사",
            "page": 125,
            "source": "읽기 전",
            "guide": "动词表达，意思是“打开”。复习时把前面的宾语或场景一起记，方便在口语中直接使用。",
            "exampleKo": "밸브를 열어 보세요.",
            "exampleZh": "打开阀门试试。"
        },
        {
            "ko": "온도를 낮추다",
            "zh": "调低温度",
            "pos": "동사구",
            "page": 125,
            "source": "읽기 전",
            "guide": "动词表达，意思是“调低温度”。复习时把前面的宾语或场景一起记，方便在口语中直接使用。",
            "tips": [
                { "label": "易错", "text": "낮다=低，낮추다=调低" }
            ],
            "exampleKo": "냉동이 안 되면 온도를 낮춰 보세요.",
            "exampleZh": "不能冷冻的话试着调低温度。"
        },
        {
            "ko": "온도를 높이다",
            "zh": "调高温度",
            "pos": "동사구",
            "page": 125,
            "source": "읽기 전",
            "guide": "动词表达，意思是“调高温度”。复习时把前面的宾语或场景一起记，方便在口语中直接使用。",
            "tips": [
                { "label": "易错", "text": "높다=高，높이다=调高" }
            ],
            "exampleKo": "방이 추우면 온도를 높이세요.",
            "exampleZh": "房间冷的话请调高温度。"
        },
        {
            "ko": "펌프질을 하다",
            "zh": "用皮搋子疏通；抽压",
            "pos": "동사구",
            "page": 125,
            "source": "읽기 전/배운 어휘 확인",
            "guide": "动词表达，意思是“用皮搋子疏通、抽压”。复习时把前面的宾语或场景一起记，方便在口语中直接使用。",
            "tips": [
                { "label": "搭配", "text": "막힌 변기에 펌프질하다" }
            ],
            "exampleKo": "변기가 막히면 펌프질을 해 보세요.",
            "exampleZh": "马桶堵了的话用皮搋子试试。"
        },
        {
            "ko": "확인하다",
            "zh": "确认",
            "pos": "동사",
            "page": 125,
            "source": "읽기 전",
            "guide": "动词表达，意思是“确认”。复习时把前面的宾语或场景一起记，方便在口语中直接使用。",
            "exampleKo": "플러그를 잘 꽂았는지 확인하세요.",
            "exampleZh": "确认插头是否插好。"
        },
        {
            "ko": "변기 세제",
            "zh": "马桶清洁剂/疏通剂",
            "pos": "명사구",
            "page": 125,
            "source": "읽기 전",
            "guide": "固定表达，意思是“马桶清洁剂/疏通剂”。建议按整块记忆，写作和口语里可以直接套用。",
            "exampleKo": "변기 세제를 사용해 보세요.",
            "exampleZh": "试着使用马桶清洁剂/疏通剂。"
        },
        {
            "ko": "화면이 안 나오다",
            "zh": "画面不显示",
            "pos": "표현",
            "page": 125,
            "source": "읽기 전",
            "guide": "固定表达，意思是“画面不显示”。建议按整块记忆，写作和口语里可以直接套用。",
            "tips": [
                { "label": "搭配", "text": "화면/소리가 안 나오다" }
            ],
            "exampleKo": "노트북 화면이 안 나와요.",
            "exampleZh": "笔记本屏幕不显示。"
        },
        {
            "ko": "앱을 종료하다",
            "zh": "关闭应用",
            "pos": "동사구",
            "page": 125,
            "source": "읽기 전",
            "guide": "动词表达，意思是“关闭应用”。复习时把前面的宾语或场景一起记，方便在口语中直接使用。",
            "tips": [
                { "label": "易错", "text": "종료하다比끄다更正式" }
            ],
            "exampleKo": "앱을 종료하거나 데이터를 삭제해 보세요.",
            "exampleZh": "关闭应用或删除数据试试。"
        },
        {
            "ko": "불필요한 데이터",
            "zh": "不必要的数据",
            "pos": "명사구",
            "page": 125,
            "source": "읽기 전",
            "guide": "固定表达，意思是“不必要的数据”。建议按整块记忆，写作和口语里可以直接套用。",
            "exampleKo": "불필요한 데이터를 삭제하세요.",
            "exampleZh": "请删除不必要的数据。"
        },
        {
            "ko": "삭제하다",
            "zh": "删除",
            "pos": "동사",
            "page": 125,
            "source": "읽기 전",
            "guide": "动词表达，意思是“删除”。复习时把前面的宾语或场景一起记，方便在口语中直接使用。",
            "exampleKo": "필요 없는 파일을 삭제했어요.",
            "exampleZh": "删除了不需要的文件。"
        },
        {
            "ko": "인터넷 게시판",
            "zh": "网络论坛/留言板",
            "pos": "명사구",
            "page": 126,
            "source": "읽기",
            "guide": "固定表达，意思是“网络论坛/留言板”。建议按整块记忆，写作和口语里可以直接套用。",
            "tips": [
                { "label": "搭配", "text": "게시판에 질문을 올리다" }
            ],
            "exampleKo": "인터넷 게시판에 질문을 올렸어요.",
            "exampleZh": "在网络论坛上发了问题。"
        },
        {
            "ko": "자주 막히다",
            "zh": "经常堵",
            "pos": "표현",
            "page": 126,
            "source": "읽기",
            "guide": "固定表达，意思是“经常堵”。建议按整块记忆，写作和口语里可以直接套用。",
            "exampleKo": "저희 집 변기가 자주 막혀요.",
            "exampleZh": "我们家马桶经常堵。"
        },
        {
            "ko": "조심하다",
            "zh": "小心",
            "pos": "동사",
            "page": 126,
            "source": "읽기",
            "guide": "动词表达，意思是“小心”。复习时把前面的宾语或场景一起记，方便在口语中直接使用。",
            "exampleKo": "사용할 때마다 조심해요.",
            "exampleZh": "每次使用都很小心。"
        },
        {
            "ko": "일주일에 서너 번",
            "zh": "一周三四次",
            "pos": "표현",
            "page": 126,
            "source": "읽기",
            "guide": "固定表达，意思是“一周三四次”。建议按整块记忆，写作和口语里可以直接套用。",
            "tips": [
                { "label": "易错", "text": "서너=三四，不是十四" }
            ],
            "exampleKo": "일주일에 서너 번 이상 막혀요.",
            "exampleZh": "一周堵三四次以上。"
        },
        {
            "ko": "뚫다",
            "zh": "疏通；打通",
            "pos": "동사",
            "page": 126,
            "source": "읽기/배운 어휘 확인",
            "guide": "动词表达，意思是“疏通、打通”。复习时把前面的宾语或场景一起记，方便在口语中直接使用。",
            "tips": [
                { "label": "易错", "text": "변기를 뚫다=疏通；구멍을 뚫다=打洞" }
            ],
            "exampleKo": "펌프질을 해서 변기를 뚫었어요.",
            "exampleZh": "用皮搋子把马桶疏通了。"
        },
        {
            "ko": "그때뿐이다",
            "zh": "只是一时有效",
            "pos": "표현",
            "page": 126,
            "source": "읽기",
            "guide": "固定表达，意思是“只是一时有效”。建议按整块记忆，写作和口语里可以直接套用。",
            "tips": [
                { "label": "易错", "text": "뿐이다前面常接名词或그때" }
            ],
            "exampleKo": "세제를 써도 그때뿐이에요.",
            "exampleZh": "用了清洁剂也只是当时有效。"
        },
        {
            "ko": "전문가 도움",
            "zh": "专家帮助",
            "pos": "명사구",
            "page": 126,
            "source": "읽기",
            "guide": "固定表达，意思是“专家帮助”。建议按整块记忆，写作和口语里可以直接套用。",
            "exampleKo": "전문가 도움을 받으셔야 합니다.",
            "exampleZh": "需要接受专家帮助。"
        },
        {
            "ko": "스트레스가 심하다",
            "zh": "压力很大",
            "pos": "표현",
            "page": 126,
            "source": "읽기",
            "guide": "固定表达，意思是“压力很大”。建议按整块记忆，写作和口语里可以直接套用。",
            "exampleKo": "자주 고장 나면 스트레스가 심해요.",
            "exampleZh": "经常坏的话压力很大。"
        },
        {
            "ko": "물의 양",
            "zh": "水量",
            "pos": "명사구",
            "page": 126,
            "source": "읽기",
            "guide": "固定表达，意思是“水量”。建议按整块记忆，写作和口语里可以直接套用。",
            "exampleKo": "물의 양이 부족해서 생기는 문제일 수 있어요.",
            "exampleZh": "可能是水量不足造成的问题。"
        },
        {
            "ko": "이물질",
            "zh": "异物",
            "pos": "명사",
            "page": 126,
            "source": "읽기/배운 어휘 확인",
            "guide": "名词，意思是“异物”。可以作为句子的主语或宾语来使用，复习时注意它常出现的生活场景。",
            "tips": [
                { "label": "搭配", "text": "이물질이 들어가다 / 끼다" }
            ],
            "exampleKo": "변기 안에 이물질이 들어갔어요.",
            "exampleZh": "马桶里进了异物。"
        },
        {
            "ko": "해결이 안 되다",
            "zh": "无法解决",
            "pos": "표현",
            "page": 126,
            "source": "읽기",
            "guide": "固定表达，意思是“无法解决”。建议按整块记忆，写作和口语里可以直接套用。",
            "tips": [
                { "label": "易错", "text": "해결하다主动，해결이 되다自然" }
            ],
            "exampleKo": "세제로는 해결이 안 됩니다.",
            "exampleZh": "用清洁剂无法解决。"
        },
        {
            "ko": "연락하다",
            "zh": "联系",
            "pos": "동사",
            "page": 126,
            "source": "읽기",
            "guide": "动词表达，意思是“联系”。复习时把前面的宾语或场景一起记，方便在口语中直接使用。",
            "tips": [
                { "label": "搭配", "text": "집주인에게 연락하다" }
            ],
            "exampleKo": "전문가에게 연락하세요.",
            "exampleZh": "请联系专家。"
        },
        {
            "ko": "집주인",
            "zh": "房东",
            "pos": "명사",
            "page": 127,
            "source": "쓰기",
            "guide": "名词，意思是“房东”。可以作为句子的主语或宾语来使用，复习时注意它常出现的生活场景。",
            "exampleKo": "집주인에게 수리를 요청했어요.",
            "exampleZh": "向房东请求维修了。"
        },
        {
            "ko": "관리 사무실",
            "zh": "管理办公室",
            "pos": "명사구",
            "page": 127,
            "source": "쓰기",
            "guide": "固定表达，意思是“管理办公室”。建议按整块记忆，写作和口语里可以直接套用。",
            "tips": [
                { "label": "搭配", "text": "관리 사무실에 연락하다" }
            ],
            "exampleKo": "관리 사무실에 연락하세요.",
            "exampleZh": "请联系管理办公室。"
        },
        {
            "ko": "요청하다",
            "zh": "请求",
            "pos": "동사",
            "page": 127,
            "source": "쓰기",
            "guide": "动词表达，意思是“请求”。复习时把前面的宾语或场景一起记，方便在口语中直接使用。",
            "tips": [
                { "label": "搭配", "text": "수리를 요청하다" }
            ],
            "exampleKo": "집수리를 요청하는 글을 써요.",
            "exampleZh": "写请求房屋维修的文字。"
        },
        {
            "ko": "메모하다",
            "zh": "做笔记",
            "pos": "동사",
            "page": 127,
            "source": "쓰기",
            "guide": "动词表达，意思是“做笔记”。复习时把前面的宾语或场景一起记，方便在口语中直接使用。",
            "exampleKo": "요청할 내용을 메모해 보세요.",
            "exampleZh": "请把要请求的内容记下来。"
        },
        {
            "ko": "보증서",
            "zh": "保修证；保证书",
            "pos": "명사",
            "page": 128,
            "source": "문화와 정보",
            "guide": "名词，意思是“保修证、保证书”。可以作为句子的主语或宾语来使用，复习时注意它常出现的生活场景。",
            "tips": [
                { "label": "搭配", "text": "보증서를 받다 / 보관하다" }
            ],
            "exampleKo": "전자 제품을 사면 보증서를 받습니다.",
            "exampleZh": "买电子产品会收到保修证。"
        },
        {
            "ko": "모델명",
            "zh": "型号名称",
            "pos": "명사",
            "page": 128,
            "source": "문화와 정보",
            "guide": "名词，意思是“型号名称”。可以作为句子的主语或宾语来使用，复习时注意它常出现的生活场景。",
            "exampleKo": "보증서에는 모델명이 있습니다.",
            "exampleZh": "保修证上有型号名称。"
        },
        {
            "ko": "구입 일자",
            "zh": "购买日期",
            "pos": "명사구",
            "page": 128,
            "source": "문화와 정보",
            "guide": "固定表达，意思是“购买日期”。建议按整块记忆，写作和口语里可以直接套用。",
            "tips": [
                { "label": "搭配", "text": "구입 일자를 기준으로 하다" }
            ],
            "exampleKo": "구입 일자는 영수증으로 확인합니다.",
            "exampleZh": "购买日期用收据确认。"
        },
        {
            "ko": "보증 내용",
            "zh": "保修内容",
            "pos": "명사구",
            "page": 128,
            "source": "문화와 정보",
            "guide": "固定表达，意思是“保修内容”。建议按整块记忆，写作和口语里可以直接套用。",
            "exampleKo": "보증 내용을 읽어 보세요.",
            "exampleZh": "请读一下保修内容。"
        },
        {
            "ko": "보증 기간",
            "zh": "保修期",
            "pos": "명사구",
            "page": 128,
            "source": "문화와 정보/배운 어휘 확인",
            "guide": "固定表达，意思是“保修期”。建议按整块记忆，写作和口语里可以直接套用。",
            "tips": [
                { "label": "搭配", "text": "보증 기간이 지나다 / 남다" }
            ],
            "exampleKo": "보증 기간은 제품에 따라 다릅니다.",
            "exampleZh": "保修期因产品而异。"
        },
        {
            "ko": "제조사",
            "zh": "制造商",
            "pos": "명사",
            "page": 128,
            "source": "문화와 정보",
            "guide": "名词，意思是“制造商”。可以作为句子的主语或宾语来使用，复习时注意它常出现的生活场景。",
            "tips": [
                { "label": "易错", "text": "제조사=制造商，판매자=销售者" }
            ],
            "exampleKo": "제조사가 무료 수리를 약속합니다.",
            "exampleZh": "制造商承诺免费维修。"
        },
        {
            "ko": "제품 판매자",
            "zh": "产品销售者",
            "pos": "명사구",
            "page": 128,
            "source": "문화와 정보",
            "guide": "固定表达，意思是“产品销售者”。建议按整块记忆，写作和口语里可以直接套用。",
            "exampleKo": "제품 판매자에게 문의하세요.",
            "exampleZh": "请咨询产品销售者。"
        },
        {
            "ko": "무료 수리",
            "zh": "免费维修",
            "pos": "명사구",
            "page": 128,
            "source": "문화와 정보",
            "guide": "固定表达，意思是“免费维修”。建议按整块记忆，写作和口语里可以直接套用。",
            "tips": [
                { "label": "易错", "text": "무료 수리=무상 수리" }
            ],
            "exampleKo": "보증 기간에는 무료 수리가 가능합니다.",
            "exampleZh": "保修期内可以免费维修。"
        },
        {
            "ko": "기준으로 하다",
            "zh": "以……为准",
            "pos": "표현",
            "page": 128,
            "source": "문화와 정보",
            "guide": "固定表达，意思是“以……为准”。建议按整块记忆，写作和口语里可以直接套用。",
            "tips": [
                { "label": "搭配", "text": "N을 기준으로 하다" }
            ],
            "exampleKo": "보증 기간은 구입 일자를 기준으로 합니다.",
            "exampleZh": "保修期以购买日期为准。"
        },
        {
            "ko": "영수증",
            "zh": "收据",
            "pos": "명사",
            "page": 128,
            "source": "문화와 정보",
            "guide": "名词，意思是“收据”。可以作为句子的主语或宾语来使用，复习时注意它常出现的生活场景。",
            "exampleKo": "영수증을 보관하세요.",
            "exampleZh": "请保管收据。"
        },
        {
            "ko": "정상적인 상태",
            "zh": "正常状态",
            "pos": "명사구",
            "page": 128,
            "source": "문화와 정보",
            "guide": "固定表达，意思是“正常状态”。建议按整块记忆，写作和口语里可以直接套用。",
            "exampleKo": "정상적인 상태에서 발생한 고장은 무상 수리가 됩니다.",
            "exampleZh": "正常状态下发生的故障可免费维修。"
        },
        {
            "ko": "발생한 고장",
            "zh": "发生的故障",
            "pos": "명사구",
            "page": 128,
            "source": "문화와 정보",
            "guide": "固定表达，意思是“发生的故障”。建议按整块记忆，写作和口语里可以直接套用。",
            "exampleKo": "제품에 발생한 고장을 점검합니다.",
            "exampleZh": "检查产品发生的故障。"
        },
        {
            "ko": "사용 설명서",
            "zh": "使用说明书",
            "pos": "명사구",
            "page": 128,
            "source": "문화와 정보",
            "guide": "固定表达，意思是“使用说明书”。建议按整块记忆，写作和口语里可以直接套用。",
            "exampleKo": "사용 설명서의 주의 사항을 지키세요.",
            "exampleZh": "请遵守使用说明书的注意事项。"
        },
        {
            "ko": "주의 사항",
            "zh": "注意事项",
            "pos": "명사구",
            "page": 128,
            "source": "문화와 정보",
            "guide": "固定表达，意思是“注意事项”。建议按整块记忆，写作和口语里可以直接套用。",
            "tips": [
                { "label": "搭配", "text": "주의 사항을 지키다" }
            ],
            "exampleKo": "주의 사항을 지키지 않으면 비용을 내야 합니다.",
            "exampleZh": "不遵守注意事项可能要付费。"
        },
        {
            "ko": "고객의 실수",
            "zh": "顾客的失误",
            "pos": "명사구",
            "page": 128,
            "source": "문화와 정보",
            "guide": "固定表达，意思是“顾客的失误”。建议按整块记忆，写作和口语里可以直接套用。",
            "tips": [
                { "label": "搭配", "text": "고객의 실수로 고장이 나다" }
            ],
            "exampleKo": "고객의 실수로 고장이 나면 비용을 지불해야 해요.",
            "exampleZh": "因顾客失误导致故障时要支付费用。"
        },
        {
            "ko": "지불하다",
            "zh": "支付",
            "pos": "동사",
            "page": 128,
            "source": "문화와 정보",
            "guide": "动词表达，意思是“支付”。复习时把前面的宾语或场景一起记，方便在口语中直接使用。",
            "tips": [
                { "label": "易错", "text": "내다较口语，지불하다较正式" }
            ],
            "exampleKo": "수리 비용을 지불해야 합니다.",
            "exampleZh": "需要支付维修费。"
        },
        {
            "ko": "스마트폰",
            "zh": "智能手机",
            "pos": "명사",
            "page": 128,
            "source": "문화와 정보",
            "guide": "名词，意思是“智能手机”。可以作为句子的主语或宾语来使用，复习时注意它常出现的生活场景。",
            "exampleKo": "스마트폰 보증 기간은 보통 1년입니다.",
            "exampleZh": "智能手机保修期通常是一年。"
        },
        {
            "ko": "일반 전자 제품",
            "zh": "普通电子产品",
            "pos": "명사구",
            "page": 128,
            "source": "문화와 정보",
            "guide": "固定表达，意思是“普通电子产品”。建议按整块记忆，写作和口语里可以直接套用。",
            "exampleKo": "일반 전자 제품은 보통 1년입니다.",
            "exampleZh": "普通电子产品通常是一年。"
        },
        {
            "ko": "에어컨",
            "zh": "空调",
            "pos": "명사",
            "page": 128,
            "source": "문화와 정보",
            "guide": "名词，意思是“空调”。可以作为句子的主语或宾语来使用，复习时注意它常出现的生活场景。",
            "exampleKo": "에어컨 같은 계절 제품은 보증 기간이 더 길 수 있어요.",
            "exampleZh": "空调等季节产品保修期可能更长。"
        },
        {
            "ko": "계절 제품",
            "zh": "季节性产品",
            "pos": "명사구",
            "page": 128,
            "source": "문화와 정보",
            "guide": "固定表达，意思是“季节性产品”。建议按整块记忆，写作和口语里可以直接套用。",
            "exampleKo": "계절 제품은 보통 2년입니다.",
            "exampleZh": "季节性产品通常是两年。"
        },
        {
            "ko": "고객 부담금",
            "zh": "顾客自付金额",
            "pos": "명사구",
            "page": 129,
            "source": "배운 어휘 확인",
            "guide": "固定表达，意思是“顾客自付金额”。建议按整块记忆，写作和口语里可以直接套用。",
            "exampleKo": "보험이 있어도 고객 부담금이 있을 수 있어요.",
            "exampleZh": "即使有保险也可能有顾客自付金额。"
        },
        {
            "ko": "환급받다",
            "zh": "获得退款/返还",
            "pos": "동사",
            "page": 129,
            "source": "배운 어휘 확인",
            "guide": "动词表达，意思是“获得退款/返还”。复习时把前面的宾语或场景一起记，方便在口语中直接使用。",
            "tips": [
                { "label": "易错", "text": "환불받다=退货退款；환급받다=返还费用" }
            ],
            "exampleKo": "수리 비용 일부를 환급받았어요.",
            "exampleZh": "获得了部分维修费返还。"
        },
        {
            "ko": "종료하다",
            "zh": "结束；关闭",
            "pos": "동사",
            "page": 129,
            "source": "배운 어휘 확인",
            "guide": "动词表达，意思是“结束、关闭”。复习时把前面的宾语或场景一起记，方便在口语中直接使用。",
            "exampleKo": "앱을 종료해 보세요.",
            "exampleZh": "试着关闭应用。"
        }
    ],
    "dialogues": [
        {
            "id": "l3-09-d1",
            "title": "와이파이가 잘 안 되는 이유 추측하기",
            "page": 121,
            "source": "문법 1 대화",
            "focus": "동/형-아서 그런지",
            "sceneZh": "朋友推测 Wi-Fi 不好的原因。",
            "lines": [
                {
                    "speaker": "미호",
                    "ko": "와이파이가 잘 안 되네요.",
                    "zh": "Wi-Fi不太好呢。",
                    "guide": "잘 안 되다 可用于机器、连接、进展“不太行”。"
                },
                {
                    "speaker": "라민",
                    "ko": "네, 지하에 있어서 그런지 와이파이 연결이 잘 안 돼요.",
                    "zh": "是啊，可能因为在地下，Wi-Fi连接不太好。",
                    "guide": "-아서 그런지 表示不确定原因的推测。"
                }
            ],
            "learningPoints": [
                "-아서 그런지 后面接结果。",
                "故障原因不确定时，这个句型比直接断定更自然。"
            ],
            "drills": [
                {
                    "promptZh": "说“可能因为头发进去了，下水口堵了”。",
                    "pattern": "-아서 그런지",
                    "answerKo": "머리카락이 들어가서 그런지 하수구가 막힌 것 같아요."
                },
                {
                    "promptZh": "说“可能因为下载的文件多，速度变慢了”。",
                    "pattern": "-아서 그런지",
                    "answerKo": "다운로드 받은 파일이 많아서 그런지 속도가 느린 것 같아요."
                }
            ]
        },
        {
            "id": "l3-09-d2",
            "title": "서비스 센터 방문 전에 예약 묻기",
            "page": 122,
            "source": "문법 2 대화",
            "focus": "동-나요?",
            "sceneZh": "去服务中心前，礼貌询问是否需要预约。",
            "lines": [
                {
                    "speaker": "이링",
                    "ko": "노트북이 고장 나서 서비스 센터에 방문하려고 하는데 예약을 해야 되나요?",
                    "zh": "笔记本坏了，我想去服务中心，需要预约吗？",
                    "guide": "해야 되나요? 是柔和地询问“需要做吗”。"
                },
                {
                    "speaker": "동료",
                    "ko": "네, 예약하고 가는 게 좋아요.",
                    "zh": "是的，预约后再去比较好。",
                    "guide": "-는 게 좋아요 表示建议。"
                }
            ],
            "learningPoints": [
                "维修咨询常用 해야 되나요?, 걸리나요?, 가능한가요?。",
                "예약하고 가다 = 预约后再去。"
            ],
            "drills": [
                {
                    "promptZh": "问“修理要多长时间？”",
                    "pattern": "동-나요?",
                    "answerKo": "수리하는 데 얼마나 걸리나요?"
                },
                {
                    "promptZh": "问“可以免费维修吗？”",
                    "pattern": "동-나요?",
                    "answerKo": "무상 수리를 받을 수 있나요?"
                }
            ]
        },
        {
            "id": "l3-09-d3",
            "title": "서비스 센터에 전화 문의하기",
            "page": 123,
            "source": "말하기",
            "focus": "서비스 문의",
            "sceneZh": "向服务中心说明笔记本故障，并请求预约。",
            "lines": [
                {
                    "speaker": "아나이스",
                    "ko": "노트북에 문제가 있어서 서비스 문의 좀 드리려고 하는데요.",
                    "zh": "我的笔记本有问题，想咨询一下服务。",
                    "guide": "-려고 하는데요 是打电话开场时常用的柔和表达。"
                },
                {
                    "speaker": "직원",
                    "ko": "네, 고객님. 어떤 문제가 있는지 말씀해 주시겠어요?",
                    "zh": "好的，顾客。能请您说明有什么问题吗？",
                    "guide": "말씀해 주시겠어요? 是 매우 공손한 요청。"
                },
                {
                    "speaker": "아나이스",
                    "ko": "어제부터 전원이 안 켜지는데 어떻게 해야 되나요?",
                    "zh": "从昨天开始电源打不开，应该怎么办呢？",
                    "guide": "어떻게 해야 되나요? 很适合咨询解决方法。"
                },
                {
                    "speaker": "직원",
                    "ko": "갑자기 문제가 생긴 건가요? 혹시 다른 문제는 없으셨어요?",
                    "zh": "是突然出现的问题吗？有没有其他问题呢？",
                    "guide": "혹시 让问题听起来更委婉。"
                },
                {
                    "speaker": "아나이스",
                    "ko": "며칠 전부터 속도가 좀 느려졌어요.",
                    "zh": "从几天前开始速度有点变慢了。",
                    "guide": "-아/어지다 表示状态变化。"
                },
                {
                    "speaker": "직원",
                    "ko": "고객님, 가까운 서비스 센터에 방문하셔서 점검을 받아 보셔야 할 것 같습니다.",
                    "zh": "顾客，您可能需要去附近的服务中心接受检查。",
                    "guide": "-셔야 할 것 같습니다 是非常礼貌的建议。"
                },
                {
                    "speaker": "아나이스",
                    "ko": "아, 그래요? 그럼 예약 좀 부탁드려요.",
                    "zh": "啊，是吗？那麻烦帮我预约一下。",
                    "guide": "부탁드려요 比 부탁해요 更尊敬。"
                }
            ],
            "learningPoints": [
                "故障电话可按“문제 있음 -> 증상 -> 언제부터 -> 요청”顺序说。",
                "서비스场景里用 고객님, 말씀, 방문하셔서, 부탁드려요 更自然。"
            ],
            "drills": [
                {
                    "promptZh": "说明“突然无法启动，想申请上门服务”。",
                    "pattern": "부팅이 안 되다 / 출장 서비스",
                    "answerKo": "갑자기 부팅이 안 되는데 출장 서비스를 신청하고 싶어요."
                },
                {
                    "promptZh": "说明“冰箱冷冻不太好，想确认冷冻室温度”。",
                    "pattern": "냉동이 잘 안 되다",
                    "answerKo": "냉동이 잘 안 되는데 냉동실 온도를 먼저 확인해야 하나요?"
                }
            ]
        }
    ],
    "readings": [
        {
            "title": "저희 집 변기가 자주 막혀요",
            "page": 126,
            "source": "읽기",
            "paragraphs": [
                {
                    "ko": "저희 집은 변기를 많이 사용하지도 않는데 너무 자주 막힙니다. 사용할 때마다 조심하는데 물이 잘 내려가지 않습니다. 일주일에 서너 번 이상 막히는 것 같습니다.",
                    "zh": "我们家也没有很频繁地使用马桶，但它太经常堵了。每次使用都很小心，可水还是不太下去。一周好像会堵三四次以上。",
                    "guide": "질문글은 문제 빈도와 현재 상황을 먼저 설명한다。"
                },
                {
                    "ko": "막힐 때마다 펌프질을 하고 변기 세제를 사용해서 뚫는데 그때뿐이고 며칠 지나면 다시 막힙니다. 어떻게 하면 좋을까요?",
                    "zh": "每次堵的时候都会用皮搋子疏通，也用马桶清洁剂/疏通剂疏通，但只是当时有效，过几天又会堵。该怎么办才好呢？",
                    "guide": "그때뿐이다 表示“只是当时有效”。"
                },
                {
                    "ko": "그 정도면 전문가 도움을 받으셔야 합니다. 일주일에 세 번 이상 막히면 사용하실 때마다 스트레스가 심하시겠어요.",
                    "zh": "这种程度的话，您需要请专家帮忙。如果一周堵三次以上，每次使用时压力应该很大吧。",
                    "guide": "回答先共情，再建议专家介入。"
                },
                {
                    "ko": "변기가 막히는 이유는 여러 가지가 있습니다. 오래 사용했거나 물의 양이 부족해서 생기는 문제일 수도 있고, 변기 안에 이물질이 들어간 경우도 생각해 볼 수 있습니다. 이물질이 들어간 경우에는 펌프질이나 세제를 사용하는 것으로는 해결이 안 됩니다. 전문가에게 연락하셔서 점검과 도움을 받으시기 바랍니다.",
                    "zh": "马桶堵的原因有很多。可能是使用时间久了，或水量不足造成的问题，也可以考虑马桶里进了异物的情况。如果进了异物，仅靠皮搋子或清洁剂无法解决。请联系专家接受检查和帮助。",
                    "guide": "原因说明用了 -거나, -일 수도 있고, 경우도 생각해 볼 수 있습니다。"
                }
            ],
            "questions": [
                {
                    "q": "어떤 문제가 있어서 인터넷 게시판에 질문을 했습니까?",
                    "zh": "因为有什么问题才在网络论坛提问？",
                    "a": "집 변기가 너무 자주 막혀서 질문했다."
                },
                {
                    "q": "변기에 이물질이 들어간 경우에는 어떻게 해야 합니까?",
                    "zh": "如果马桶里进了异物，该怎么办？",
                    "a": "전문가에게 연락해서 점검과 도움을 받아야 한다."
                },
                {
                    "q": "변기가 막히는 이유가 아닌 것은 무엇입니까?",
                    "zh": "不属于马桶堵塞原因的是什么？",
                    "a": "펌프질을 자주 해서."
                }
            ]
        }
    ],
    "culture": {
        "titleKo": "전자 제품 보증 기간",
        "titleZh": "电子产品保修期",
        "page": 128,
        "source": "문화와 정보",
        "summaryZh": "这篇文化文章介绍韩国电子产品保修期：保修证记录型号、购买日期和保修内容；正常使用中发生的故障可免费维修，但顾客失误造成的故障需要付费。",
        "paragraphs": [
            {
                "ko": "전자 제품을 사면 보증서를 받는다. 보증서에는 모델명, 구입 일자, 보증 내용, 보증 기간 등이 있다. 제품 보증 기간이란 제조사나 제품 판매자가 소비자에게 무료 수리를 약속한 기간을 말한다. 이때 보증 기간은 구입 일자를 기준으로 하며, 구입 일자는 제품 보증서나 영수증으로 확인한다.",
                "zh": "购买电子产品时会收到保修证。保修证上有型号名称、购买日期、保修内容、保修期等。产品保修期是指制造商或产品销售者向消费者承诺免费维修的期间。此时保修期以购买日期为准，购买日期可通过产品保修证或收据确认。",
                "guide": "第一段说明保修证内容和保修期计算 기준。"
            },
            {
                "ko": "보증서가 있다고 해서 보증 기간 내에 언제나 무상으로 수리를 받는 것은 아니다. 보증 기간 내에 정상적인 상태에서 발생한 고장에 대해서는 무상 수리가 가능하지만 사용 설명서의 주의 사항을 지키지 않았거나 고객의 실수로 고장이 난 경우에는 수리 비용을 지불해야 한다.",
                "zh": "并不是说有保修证，在保修期内就总能免费维修。保修期内正常状态下发生的故障可以免费维修，但如果没有遵守使用说明书的注意事项，或者因顾客失误导致故障，就必须支付维修费用。",
                "guide": "핵심 판단 기준은 정상 사용인지, 고객 실수인지。"
            },
            {
                "ko": "보증 기간은 제품에 따라 다른데 보통 스마트폰이나 일반 전자 제품은 1년, 에어컨 등 계절 제품은 2년이다.",
                "zh": "保修期因产品而异，通常智能手机或普通电子产品为1年，空调等季节性产品为2年。",
                "guide": "最后给出常见保修期例子。"
            }
        ],
        "keyTerms": [
            {
                "ko": "보증서",
                "zh": "记载型号、购买日期、保修内容和保修期的凭证。"
            },
            {
                "ko": "보증 기간",
                "zh": "制造商或销售者承诺免费维修的期间。"
            },
            {
                "ko": "무상 수리",
                "zh": "不支付维修费的免费维修。"
            },
            {
                "ko": "고객의 실수",
                "zh": "因使用者失误造成的故障，通常需要付费。"
            }
        ],
        "questions": [
            {
                "q": "전자 제품의 보증서에는 어떤 정보가 있습니까?",
                "zh": "电子产品保修证上有哪些信息？",
                "a": "모델명, 구입 일자, 보증 내용, 보증 기간 등이 있다."
            },
            {
                "q": "어느 경우에 무상 수리를 받을 수 있습니까?",
                "zh": "什么情况下可以免费维修？",
                "a": "보증 기간 내에 정상적인 상태에서 발생한 고장일 때 가능하다."
            },
            {
                "q": "여러분이 가지고 있는 물건의 보증 기간은 얼마나 됩니까?",
                "zh": "你拥有的物品保修期是多久？",
                "a": "스마트폰, 노트북, 에어컨 등 실제 물건을 예로 들어 말해 보자."
            }
        ]
    },
    "practice": [
        {
            "type": "grammar",
            "title": "-아서 그런지로 이유 추측하기",
            "prompt": "“可能因为手机掉到地上，所以电源打不开”를 한국어로 말해 보세요.",
            "answer": "휴대 전화를 바닥에 떨어뜨려서 그런지 전원이 안 켜져요."
        },
        {
            "type": "grammar",
            "title": "부드럽게 질문하기",
            "prompt": "“需要预约吗？”를 서비스 센터에 묻는 말로 바꿔 보세요.",
            "answer": "예약을 해야 되나요?"
        },
        {
            "type": "speaking",
            "title": "고장 설명하기",
            "prompt": "用三句话说明笔记本电脑的故障症状。（韩语题目：노트북 증상을 세 문장으로 말해 보세요.）",
            "answer": "노트북에 문제가 있어요. 어제부터 전원이 안 켜져요. 며칠 전부터 속도도 느려졌어요."
        },
        {
            "type": "vocabulary",
            "title": "집 고장 표현",
            "prompt": "用 하수구、변기、가스불、전등 造故障句。（韩语题目：하수구, 변기, 가스불, 전등으로 고장 문장을 만들어 보세요.）",
            "answer": "하수구가 막혔어요. 변기가 막혔어요. 가스불이 안 들어와요. 전등이 나갔어요."
        },
        {
            "type": "reading",
            "title": "게시판 답변 이해",
            "prompt": "阅读理解：异物进入马桶时，为什么需要专家帮助？（韩语题目：변기에 이물질이 들어간 경우 왜 전문가 도움이 필요합니까?）",
            "answer": "펌프질이나 세제만으로는 해결이 안 되기 때문입니다."
        },
        {
            "type": "culture",
            "title": "보증 기간 판단하기",
            "prompt": "文化理解：即使在保修期内，什么时候也需要支付维修费？（韩语题目：보증 기간 안이어도 수리 비용을 내야 하는 경우는 언제입니까?）",
            "answer": "사용 설명서의 주의 사항을 지키지 않았거나 고객의 실수로 고장이 난 경우입니다."
        },
        {
            "type": "writing",
            "title": "집 수리 요청 글쓰기",
            "prompt": "写作：包含问题、开始时间、请求事项，给管理办公室写维修请求。（韩语题目：문제, 시작 시점, 요청 사항을 넣어 관리 사무실에 쓰세요.）",
            "answer": "안녕하세요. 며칠 전부터 화장실 물이 잘 내려가지 않습니다. 점검과 수리를 부탁드립니다."
        }
    ],
    "translationGuide": [
        {
            "title": "-아서 그런지 的不确定感",
            "tip": "它不是确定原因，而是“可能因为……”。中文可译为“也许/可能是因为……”。"
        },
        {
            "title": "서비스 문의 电话顺序",
            "tip": "先说 제품/문제，再说 언제부터, 接着问 어떻게 해야 되나요 或 예약을 해야 되나요。"
        },
        {
            "title": "보증 기간 不等于一定免费",
            "tip": "文章重点是“保修期内正常使用故障免费，顾客失误或违反说明书则付费”。"
        }
    ]
};
